"""
combined_pinn_lnn_ukdale.py
============================
Dual-head PINN + Gated-Detection LNN for UKDALE NILM.

Architecture
------------
The two previously separate models are merged into a single end-to-end network:

    Input (batch, WIN, 8)   — 8-channel mains features (same as FixedPINNLNN)
           ↓
    ...
    LayerNorm(hidden*2)
           ↓ (per appliance)
    Gate  head: Linear → sigmoid  →  g ∈ [0,1]   (detection confidence)
    Power head: Linear → sigmoid  →  p ∈ [0,1]   (raw power estimate)
           ↓
    gated_power = g × p     ← the combination
...
directly fixing the precision problem (false positives). The physics loss
acts on gated_power, so it also benefits from the suppression.
...
Loss schedule:
  ...
  Full   (WARMUP_EPOCHS +):       MSE + λ_phys * L_phys + λ_event * L_event

Default dataset: medium_dataset  (432k train / 100.8k val / 100.8k test)
"""
...
DEFAULT_DATASET_DIR = os.path.join(os.path.dirname(__file__), 'medium_dataset')


# ---------------------------------------------------------------------------
# Adaptive thresholds  (identical to fixed_pinn_lnn_ukdale.py)
# ---------------------------------------------------------------------------
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading UKDALE CSV data from '{dataset_dir}' ...")
...
# ---------------------------------------------------------------------------
# Combined model  — the key new class
# ---------------------------------------------------------------------------
...
class CombinedPINNAdvancedLNN(nn.Module):
    ...
    gate_heads  produce detection confidence g ∈ [0,1] at every timestep.
    power_heads produce raw power estimate   p ∈ [0,1] at every timestep.
    Output:  gated_power = g × p  (batch, WIN, n_apps)
    ...
    def __init__(self, in_ch: int, hidden: int, n_apps: int, dt: float = 0.1):
        ...
        # Power regression heads — sigmoid output ∈ [0,1]
...
class PhysicsConsistencyLoss(nn.Module):
    """ReLU(Σ gated_power_raw - P_agg_raw - ε) — acts on the gated estimate."""
...
class GatedEventLoss(nn.Module):
    ...
    pos_weight compensates for the ON fraction being very small (~2%).
    """
...
def train(data_dict: dict, save_dir: str,
    ...
    print(f"Model: CombinedPINNAdvancedLNN  (gated power = gate × power)\n")
    ...
    cfg = {
        ...
        'description': 'gated power = sigmoid(gate_logit) × sigmoid(power_logit)',
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN — Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_loss.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN — Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_per_appliance.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = f'models/combined_pinn_lnn_{timestamp}'
