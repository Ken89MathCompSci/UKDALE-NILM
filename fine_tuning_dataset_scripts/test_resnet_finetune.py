"""
ResNet — Cross-House Fine-Tuning on fine_tuning_dataset/
...
def train_on_appliance(splits, appliance, dataset_dir=DEFAULT_DATASET_DIR,
                       ...
                       save_dir='models/resnet_finetune'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    for epoch in range(EPOCHS):
        ...
        if va_loss < best_val:
            ...
            save_model(model,
                       {'input_size':1,'output_size':1,'base_width':base_width,'layers':layers},
    ...
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'resnet_{appliance}_metrics.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    config = {
        'appliance': appliance, 'dataset': 'fine_tuning_dataset', 'model': 'ResNet',
    ...
    with open(os.path.join(save_dir, f'resnet_{appliance}_results.json'),
...
def run_all(dataset_dir=DEFAULT_DATASET_DIR, base_width=32, layers=None):
    if layers is None:
        layers = [2, 2, 2]
    ...
    for app in APPLIANCES:
        ...
        try:
            _, _, after, before = train_on_appliance(
                ...
                base_width=base_width, layers=layers, save_dir=app_dir)
            all_results[app] = {
    ...
    summary = {
        'timestamp': timestamp, 'model': 'ResNet', 'dataset': 'fine_tuning_dataset',
        'dataset_splits': {
    ...
    print(f"\nResNet fine-tuning complete. Results saved to {base_dir}")
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    run_all(args.dataset_dir, args.base_width)
