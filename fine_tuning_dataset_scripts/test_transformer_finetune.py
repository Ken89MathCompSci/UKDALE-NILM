"""
Transformer — Cross-House Fine-Tuning on fine_tuning_dataset/
...
def train_on_appliance(splits, appliance, dataset_dir=DEFAULT_DATASET_DIR,
                       ...
                       save_dir='models/transformer_finetune'):
    ...
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'transformer_{appliance}_metrics.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    config = {
        'appliance': appliance, 'dataset': 'fine_tuning_dataset', 'model': 'Transformer',
    ...
    with open(os.path.join(save_dir, f'transformer_{appliance}_results.json'),
...
def run_all(dataset_dir=DEFAULT_DATASET_DIR, hidden_size=128, num_layers=3,
            num_heads=4, dropout=0.1):
    print("Loading fine_tuning_dataset splits...")
    splits    = load_splits(dataset_dir)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_dir  = f"models/transformer_finetune_{timestamp}"
    all_results = {}
    ...
    summary = {
        'timestamp': timestamp, 'model': 'Transformer', 'dataset': 'fine_tuning_dataset',
        'dataset_splits': {
    ...
    print(f"\nTransformer fine-tuning complete. Results saved to {base_dir}")
