"""
Advanced LNN (AdvancedLiquidNetworkModel) -- Cross-House Fine-Tuning on fine_tuning_dataset/
...
def train_on_appliance(splits, appliance, dataset_dir=DEFAULT_DATASET_DIR,
                       ...
                       save_dir='models/advanced_lnn_finetune'):
    ...
    model = AdvancedLiquidNetworkModel(input_size=1, hidden_size=hidden_size,
    ...
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'advanced_lnn_{appliance}_metrics.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    # -- JSON -----------------------------------------------------------------
    config = {
        'appliance': appliance, 'dataset': 'fine_tuning_dataset',
    ...
    with open(os.path.join(save_dir, f'advanced_lnn_{appliance}_results.json'),
...
def run_all(dataset_dir=DEFAULT_DATASET_DIR, hidden_size=64, num_layers=2, dt=0.1):
    ...
    base_dir  = f"models/advanced_lnn_finetune_{timestamp}"
    all_results = {}
    ...
    summary = {
        'timestamp': timestamp, 'model': 'AdvancedLiquidNetworkModel',
    ...
    print(f"\nAdvanced LNN fine-tuning complete. Results saved to {base_dir}")
