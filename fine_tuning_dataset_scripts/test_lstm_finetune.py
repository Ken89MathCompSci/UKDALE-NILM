"""
LSTM -- Cross-House Fine-Tuning on fine_tuning_dataset/
...
def train_on_appliance(splits, appliance, dataset_dir=DEFAULT_DATASET_DIR,
    ...
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'lstm_{appliance}_metrics.png'), dpi=150, bbox_inches='tight')
    plt.close()

    # -- JSON -----------------------------------------------------------------
    config = {
        'appliance': appliance, 'dataset': 'fine_tuning_dataset', 'model': 'LSTM',
    ...
    with open(os.path.join(save_dir, f'lstm_{appliance}_results.json'),
...
def run_all(dataset_dir=DEFAULT_DATASET_DIR, hidden_size=64, num_layers=2, dropout=0.2):
    print("Loading fine_tuning_dataset splits...")
    splits    = load_splits(dataset_dir)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_dir  = f"models/lstm_finetune_{timestamp}"
    all_results = {}
    ...
    summary = {
        'timestamp': timestamp, 'model': 'LSTM', 'dataset': 'fine_tuning_dataset',
        'dataset_splits': {
    ...
    print(f"\nLSTM fine-tuning complete. Results saved to {base_dir}")
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    run_all(args.dataset_dir, args.hidden_size, args.num_layers, args.dropout)
