"""
Advanced LNN v4 — Per-Appliance Cross-House Fine-Tuning

Targeted improvement over v3: bounded sigmoid tau instead of unbounded softplus tau.

v3 tau (unbounded softplus):
    τ_t = τ_min + softplus(τ_base + W_τ [x_t, h_t] + b_τ)

Problem:
    softplus output is unbounded upward.  On a small fine-tune set (2h of
    House 5 data) the model can drive τ → very large values, collapsing the
    decay term -h/τ → 0 and making the hidden state nearly constant.  This
    is an overfitting failure mode specific to few-shot adaptation.

v4 tau (bounded sigmoid):
    τ_t = τ_min + (τ_max - τ_min) × σ(W_τ [x_t, h_t] + b_τ)

    τ_t ∈ (τ_min, τ_max)  strictly, regardless of input magnitude.

Benefits:
  1. Hard upper bound τ_max=5.0 — decay term never collapses; prevents the
     hidden state from becoming unresponsive after fine-tuning.
  2. Hard lower bound τ_min=0.05 — slightly higher than v3 (0.01); prevents
     too-fast decay that would wash out long-cycle appliance memory.
  3. Still depends on [x_t, h_t] — retains the hidden-state-adaptive tau
     that is the key upgrade over v1/v2.
  4. σ output interpretation: σ→0 selects τ_min (fast decay, reactive),
     σ→1 selects τ_max (slow decay, persistent memory).  The model learns
     which regime each appliance needs per-state.

Comparison across versions:
  Property              | v1 (models.py)                | v3                          | v4
  ----------------------|-------------------------------|-----------------------------|-----------------------------
  tau depends on x_t    | yes                           | yes                         | yes
  tau depends on h_t    | no                            | yes                         | yes
  tau bounded above     | no (softplus * sigmoid ≤ inf) | no (softplus unbounded)     | yes  τ < τ_max = 5.0
  tau bounded below     | clamp(1e-3)                   | τ_min + softplus > τ_min    | τ_min = 0.05
  overfit risk on small FT | low (only x_t)             | medium (unbounded growth)   | low  (hard bounds)
  expressive range      | [1e-3, ∞) but shrink-only     | (τ_min, ∞) grow and shrink  | (τ_min, τ_max) grow and shrink

Expected per-appliance behaviour:
  Dishwasher      → long multi-stage cycles;   τ biased toward upper range
  Washing machine → hour-long cycles;          τ biased toward upper range
  Fridge          → compressor duty cycles;    τ moderate, duty-cycle driven
  Microwave       → short bursty events;       τ biased toward lower range

Everything else (3-phase pipeline, scalers, metrics, plots, JSON) is identical
to test_advanced_lnn_finetune.py and v3.
"""
...
TAU_MIN = 0.05   # lower bound on τ — slightly higher than v3 to preserve long-cycle memory
TAU_MAX = 5.0    # upper bound on τ — prevents decay collapse on small fine-tune sets


# ---------------------------------------------------------------------------
# Model: bounded adaptive tau LNN
# ---------------------------------------------------------------------------

class BoundedAdaptiveLiquidTimeLayer(nn.Module):
    """
    LNN cell with bounded, hidden-and-input-adaptive tau.
    ...
    Update rule:
        ...
        τ_t       = τ_min + (τ_max − τ_min) × σ(W_τ combined + b_τ)
        h_{t+1}   = h_t + dt(−h_t/τ_t + gate·f_t)

    τ_t is strictly in (τ_min, τ_max) for all inputs.
    σ→0: fast decay (reactive, short memory)  → τ ≈ τ_min
    σ→1: slow decay (persistent memory)       → τ ≈ τ_max
    """
    ...
    def __init__(self, input_size, hidden_size, dt=0.1,
        ...
        # tau_net and gate both take [x, h]
    ...
    def forward(self, x, hidden=None):
        ...
        # bounded tau: strictly in (tau_min, tau_max)
        tau  = self.tau_min + (self.tau_max - self.tau_min) * torch.sigmoid(
            self.tau_net(combined)
        )
...
class BoundedAdaptiveLNNModel(nn.Module):
    """Stacks N BoundedAdaptiveLiquidTimeLayer cells; final hidden state → output."""

    def __init__(self, input_size, hidden_size, output_size,
                 num_layers=2, dt=0.1, tau_min=TAU_MIN, tau_max=TAU_MAX):
        super().__init__()
        self.num_layers  = num_layers
        self.hidden_size = hidden_size

        self.liquid_layers = nn.ModuleList([
            BoundedAdaptiveLiquidTimeLayer(
                input_size if i == 0 else hidden_size,
                hidden_size, dt, tau_min, tau_max)
...
def train_on_appliance(splits, appliance, dataset_dir=DEFAULT_DATASET_DIR,
                       ...
                       tau_min=TAU_MIN, tau_max=TAU_MAX,
                       save_dir='models/advanced_lnn_v4_finetune'):
    ...
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'advanced_lnn_v4_{appliance}_metrics.png'),
    ...
    config = {
        ...
        'model':     'BoundedAdaptiveLNNModel (Advanced LNN v4)',
        'tau_parameterisation': {
            'formula': 'tau_min + (tau_max - tau_min) * sigmoid(tau_net([x,h]))',
            'tau_min': tau_min,
            'tau_max': tau_max,
            'vs_v3':  'tau_min + softplus(tau_base + tau_mod([x,h]))  (unbounded)',
            'vs_v1':  'softplus(tau_base) * sigmoid(tau_mod(x))       (x-only, shrink-only)',
        },
    ...
    with open(os.path.join(save_dir, f'advanced_lnn_v4_{appliance}_results.json'),
...
def run_all(dataset_dir=DEFAULT_DATASET_DIR, hidden_size=64, num_layers=2,
            dt=0.1, tau_min=TAU_MIN, tau_max=TAU_MAX):
    print("Loading fine_tuning_dataset splits...")
    splits    = load_splits(dataset_dir)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    base_dir  = f'models/advanced_lnn_v4_finetune_{timestamp}'
    all_results = {}
    ...
    print(f"\nAdvanced LNN v4 complete.  Results → {base_dir}")
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(
        description='Advanced LNN v4: bounded sigmoid tau in [tau_min, tau_max], '
                    'depends on both x and h')
    ...
    p.add_argument('--tau-min',      type=float, default=TAU_MIN,
                   help='Minimum time constant (default 0.05)')
    p.add_argument('--tau-max',      type=float, default=TAU_MAX,
                   help='Maximum time constant (default 5.0)')
    args = p.parse_args()
    run_all(args.dataset_dir, args.hidden_size, args.num_layers,
            args.dt, args.tau_min, args.tau_max)
