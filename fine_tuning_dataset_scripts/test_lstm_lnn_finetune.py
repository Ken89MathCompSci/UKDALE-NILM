"""
LSTM-LNN — Cross-House Fine-Tuning on fine_tuning_dataset/
...
def train_on_appliance(splits, appliance, dataset_dir=DEFAULT_DATASET_DIR,
                       lstm_hidden=64, num_lstm_layers=2, hidden_size=64,
                       dropout=0.2, dt=0.1, save_dir='models/lstm_lnn_finetune'):
    ...
    model = LSTMLiquidNetworkModel(
        ...
        dropout=dropout, dt=dt).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        m = _metrics(ys.inverse_transform(vt).flatten(), ys.inverse_transform(vo).flatten())
        ...
        if va_loss < best_val:
            ...
            save_model(model,
                       {'input_size':1,'output_size':1,'lstm_hidden':lstm_hidden,
    ...
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'lstm_lnn_{appliance}_metrics.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    config = {
        'appliance': appliance, 'dataset': 'fine_tuning_dataset', 'model': 'LSTM-LNN',
        'model_params': {'lstm_hidden':lstm_hidden,'num_lstm_layers':num_lstm_layers,
    ...
    with open(os.path.join(save_dir, f'lstm_lnn_{appliance}_results.json'),
...
def run_all(dataset_dir=DEFAULT_DATASET_DIR, lstm_hidden=64, num_lstm_layers=2,
            hidden_size=64, dropout=0.2, dt=0.1):
    print("Loading fine_tuning_dataset splits...")
    splits    = load_splits(dataset_dir)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_dir  = f"models/lstm_lnn_finetune_{timestamp}"
    all_results = {}
    ...
    for app in APPLIANCES:
        ...
        try:
            _, _, after, before = train_on_appliance(
                ...
                lstm_hidden=lstm_hidden, num_lstm_layers=num_lstm_layers,
                hidden_size=hidden_size, dropout=dropout, dt=dt, save_dir=app_dir)
            all_results[app] = {
    ...
    summary = {
        'timestamp': timestamp, 'model': 'LSTM-LNN', 'dataset': 'fine_tuning_dataset',
        'dataset_splits': {
    ...
    print(f"\nLSTM-LNN fine-tuning complete. Results saved to {base_dir}")
...
if __name__ == '__main__':
    ...
    p.add_argument('--num-lstm-layers',  type=int,   default=2)
    p.add_argument('--hidden-size',      type=int,   default=64)
    p.add_argument('--dt',               type=float, default=0.1)
    args = p.parse_args()
    run_all(args.dataset_dir, args.lstm_hidden, args.num_lstm_layers,
            args.hidden_size, dt=args.dt)
