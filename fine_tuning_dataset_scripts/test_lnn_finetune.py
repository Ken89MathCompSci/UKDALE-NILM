"""
LNN (LiquidNetworkModel) -- Cross-House Fine-Tuning on fine_tuning_dataset/
...
def train_on_appliance(splits, appliance, dataset_dir=DEFAULT_DATASET_DIR,
    ...
    model = LiquidNetworkModel(input_size=1, hidden_size=hidden_size,
                               output_size=1, dt=dt).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if va_loss < best_val:
            ...
            save_model(model,
                       {'input_size':1,'output_size':1,'hidden_size':hidden_size,'dt':dt},
    ...
    plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'lnn_{appliance}_metrics.png'), dpi=150, bbox_inches='tight')
    plt.close()

    # -- JSON -----------------------------------------------------------------
    config = {
        'appliance': appliance, 'dataset': 'fine_tuning_dataset', 'model': 'LiquidNetworkModel',
    ...
    with open(os.path.join(save_dir, f'lnn_{appliance}_results.json'),
...
def run_all(dataset_dir=DEFAULT_DATASET_DIR, hidden_size=64, dt=0.1):
    ...
    base_dir  = f"models/lnn_finetune_{timestamp}"
    all_results = {}
    ...
    summary = {
        'timestamp': timestamp, 'model': 'LiquidNetworkModel', 'dataset': 'fine_tuning_dataset',
        'dataset_splits': {
    ...
    print(f"\nLNN fine-tuning complete. Results saved to {base_dir}")
