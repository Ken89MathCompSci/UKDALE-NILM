"""
Physics-Informed LNN (PINN-LNN) for NILM — UKDALE medium dataset.

Based on test_pinn_lnn_ukdale_specific_splits_minonoff.py with the default
dataset directory set to 'medium_dataset' (432,000 train / 100,800 val /
100,800 test rows at 6 s resolution, ~1 month train, ~1 week each val/test).

Architecture:
    Input (batch, WIN, 1)  — scaled mains window
         ↓
    Shared AdvancedLiquidTimeLayer (adaptive tau, input gate)
         ↓
    LayerNorm(hidden)
         ↓
    ┌──────────┬──────┬───────────┬─────────────────┐
    ...
    └──────────┴──────┴───────────┴─────────────────┘
    output: (batch, 4) — midpoint prediction

Loss:
    Warmup (epochs 0-19): MSE + L1-sparsity
    Full   (epoch 20+):   MSE + λ_phys·L_phys + L1-sparsity + BCE(dishwasher, fridge)

Min-ON/OFF post-processing (6 s timesteps):
    dishwasher      min_on=300 (30 min)   min_off=300 (30 min)
    fridge          min_on=10  (1 min)    min_off=2   (12 s)
    microwave       min_on=2   (12 s)     min_off=5   (30 s)
    washing_machine min_on=300 (30 min)   min_off=27  (162 s)
"""
...
def apply_min_on_off(binary, min_on, min_off):
    """
    Filter a binary ON/OFF sequence by removing short events.

    Pass 1: remove ON runs shorter than min_on  → set to OFF.
    Pass 2: fill  OFF gaps shorter than min_off → set to ON.
    """
    result = binary.copy().astype(np.int32)
    n = len(result)
...
def load_data(dataset_dir=DEFAULT_DATASET_DIR):
    ...
    for split in ('train', 'validation', 'test'):
        ...
        splits[split] = df
        print(f"  {split:12s}: {len(df):>7,} rows  "
...
# ---------------------------------------------------------------------------
# Per-appliance metrics with min ON/OFF filtering
# ---------------------------------------------------------------------------

def compute_per_appliance_metrics(y_true, y_pred, y_scalers):
    """
    Inverse-scale predictions, apply min-ON/OFF filter to the binary mask,
    then compute F1/precision/recall/TP/TN/FP/FN.  MAE/SAE use the raw
    continuous predictions (unfiltered).
    """
    metrics = {}
    for i, app in enumerate(APPLIANCES):
        ...
        true_bin = (raw_true > THRESHOLDS[app]).astype(np.int32)
        pred_bin = (raw_pred > THRESHOLDS[app]).astype(np.int32)
        pred_bin_filtered = apply_min_on_off(pred_bin, MIN_ON[app], MIN_OFF[app])

        m['TP'] = int(( true_bin.astype(bool) &  pred_bin_filtered.astype(bool)).sum())
        m['TN'] = int((~true_bin.astype(bool) & ~pred_bin_filtered.astype(bool)).sum())
        m['FP'] = int((~true_bin.astype(bool) &  pred_bin_filtered.astype(bool)).sum())
        m['FN'] = int(( true_bin.astype(bool) & ~pred_bin_filtered.astype(bool)).sum())

        tp, fp, fn = m['TP'], m['FP'], m['FN']
        m['precision'] = tp / (tp + fp) if (tp + fp) > 0 else 0.0
        m['recall']    = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        pr, re         = m['precision'], m['recall']
        m['f1']        = 2 * pr * re / (pr + re) if (pr + re) > 0 else 0.0
...
def train_pinn_model(data_dict, save_dir,
    ...
    print(f"Warmup: MSE+sparsity for {WARMUP_EPOCHS} epochs, then + physics + BCE.\n")
    ...
    print(f"\n{'Appliance':<18} {'F1':>8} {'Precision':>10} {'Recall':>8} "
          f"{'MAE':>8} {'SAE':>8} {'TP':>8} {'FP':>8} {'FN':>8}")
    print("-" * 95)
    for app in APPLIANCES:
        m = test_metrics[app]
    ...
    config = {
        ...
        'model': 'PhysicsInformedLiquidNetworkModel',
        'description': 'shared LiquidCell encoder + per-appliance heads + L_phys + min-ON/OFF filter',
        ...
        'test_metrics': {
            app: {k: (int(v) if isinstance(v, (np.integer, int)) else float(v))
                  for k, v in m.items()}
            for app, m in test_metrics.items()
        },
    }
    with open(os.path.join(save_dir, 'pinn_lnn_medium_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_medium_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN Medium Dataset — Per-Appliance Val Metrics', fontsize=13)
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        axes[row][0].set_title(f'{app} — F1 (min-ON/OFF filtered)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_medium_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_medium_{timestamp}"
