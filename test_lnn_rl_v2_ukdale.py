...
Extends the base PPO script with three targeted reward-shaping ideas:
  ...
  1. Asymmetric FP penalty
     Predicting ON when the appliance is truly OFF incurs a penalty scaled
     by  FP_PENALTY_SCALE × (pred_watts / threshold).  This asymmetry makes
     the agent more conservative: it prefers missing an event (FN) over
     falsely triggering one (FP), which empirically reduces the always-ON
     collapse seen with symmetric MAE.

  2. Conservation guardrail
  ...
  3. Zero-reward bonus
     When the agent correctly predicts "near-zero" (below threshold) for an
     appliance that is truly off, it receives a small positive reward bonus
     of ZERO_BONUS_W per appliance.  This provides a non-zero gradient
     signal in the (true-OFF, pred-OFF) cell, countering the tendency of
     purely negative reward functions to reward doing nothing by being silent
     about correct OFF predictions.
...
FP_PENALTY_SCALE = 5.0  # asymmetric FP cost multiplier
BETA_CONSERVE    = 1.0  # conservation guardrail weight
ZERO_BONUS_W     = 0.5  # per-appliance bonus (W-equivalent) for correct near-zero
...
# ---------------------------------------------------------------------------
# Reward  (v2: asymmetric FP + conservation + zero-bonus)
# ---------------------------------------------------------------------------

def compute_rewards_batch(actions, y_true, prev_actions, y_mins, y_ranges, agg_watts):
    """
    Vectorised reward with three v2 shaping terms.
    ...
    # ── 1. Asymmetric FP penalty ──────────────────────────────────────────
    # Predicting ON when true is OFF costs FP_PENALTY_SCALE times the
    # wattage ratio (pred / threshold), so high-power false alarms hurt more.
    fp_penalty = np.zeros(N, dtype=np.float32)
    for i in range(len(APPLIANCES)):
        fp_mask = (pred_raw[:, i] > THRESHOLD_ARR[i]) & (true_raw[:, i] <= THRESHOLD_ARR[i])
        fp_penalty += np.where(
            fp_mask,
            FP_PENALTY_SCALE * pred_raw[:, i] / THRESHOLD_ARR[i],
            0.0,
        )

    # ── 2. Conservation guardrail ─────────────────────────────────────────
    # Σ predicted appliances must not exceed aggregate × 1.1.
    # Violations are penalised proportionally (in Watts).
    conservation = np.maximum(
        0.0,
        pred_raw.sum(axis=1) - agg_watts * 1.1,
    )

    # ── 3. Zero-reward bonus ──────────────────────────────────────────────
    # Each appliance that is correctly predicted as OFF (below threshold)
    # while truly being OFF contributes a small positive reward.
    zero_bonus = np.zeros(N, dtype=np.float32)
    for i in range(len(APPLIANCES)):
        zero_mask = (pred_raw[:, i] <= THRESHOLD_ARR[i]) & (true_raw[:, i] <= THRESHOLD_ARR[i])
        zero_bonus += np.where(zero_mask, ZERO_BONUS_W, 0.0)

    reward = -(
        ALPHA       * mae_term
        + BETA_TRANS  * trans_term
        + fp_penalty
        + BETA_CONSERVE * conservation
        - zero_bonus           # bonus reduces the penalty (subtracting a negative reward)
    ) / NORM_POWER
    return reward
...
def train_model(data_splits, save_dir, hidden_size=64, dt=0.1):
    ...
    print(f"v2 reward: FP_scale={FP_PENALTY_SCALE}  β_conserve={BETA_CONSERVE}  "
          f"zero_bonus={ZERO_BONUS_W}W")
...
def parse_args():
    ...
    p.add_argument('--fp-scale',       type=float, default=FP_PENALTY_SCALE,
                   help='Asymmetric FP penalty multiplier (default 5)')
    p.add_argument('--beta-conserve',  type=float, default=BETA_CONSERVE,
                   help='Conservation guardrail weight (default 1.0)')
    p.add_argument('--zero-bonus',     type=float, default=ZERO_BONUS_W,
                   help='Per-appliance zero-reward bonus in W-equivalent (default 0.5)')
