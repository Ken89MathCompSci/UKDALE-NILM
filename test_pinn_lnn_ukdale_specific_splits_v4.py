"""
PINN-LNN UKDALE specific splits v4.
...
Changes from v3:
  ...
  6. Stitches overlapping window predictions into a single timeline before
...
def load_new_dataset(dataset_dir):
    ...
    print(f"Loading UKDALE v4 data from: {dataset_dir}")
...
def sequence_start_indices(n_rows, window_size=WIN, stride=STRIDE):
...
def stitch_scaled_predictions(window_preds, starts, n_rows, window_size=WIN):
    """
    Average overlapping seq2seq window predictions back onto the original
    timeline.

    Args:
        window_preds: (n_windows, WIN, n_apps), scaled predictions
        starts:       (n_windows,), source start index for each window
        n_rows:       full split length
    Returns:
        timeline predictions: (n_rows, n_apps), scaled
    """
    n_apps = window_preds.shape[-1]
    sums = np.zeros((n_rows, n_apps), dtype=np.float64)
    counts = np.zeros((n_rows, 1), dtype=np.float64)

    for window_idx, start in enumerate(starts):
        end = start + window_size
        sums[start:end] += window_preds[window_idx]
        counts[start:end] += 1.0

    valid = counts[:, 0] > 0
    stitched = np.zeros((n_rows, n_apps), dtype=np.float32)
    stitched[valid] = (sums[valid] / counts[valid]).astype(np.float32)
    stitched[~valid] = np.nan
    return stitched


def inverse_scale_timeline(values, y_scalers):
    raw = np.zeros_like(values, dtype=np.float32)
    for i, scaler in enumerate(y_scalers):
        valid = ~np.isnan(values[:, i])
        raw[:, i] = np.nan
        raw[valid, i] = scaler.inverse_transform(
            values[valid, i : i + 1]
        ).reshape(-1)
    return raw


def compute_stitched_metrics(split_df, window_preds, y_scalers):
    starts = sequence_start_indices(len(split_df), WIN, STRIDE)
    stitched_scaled = stitch_scaled_predictions(window_preds, starts, len(split_df), WIN)
    stitched_raw = inverse_scale_timeline(stitched_scaled, y_scalers)
    true_raw = split_df[APPLIANCES].values.astype(np.float32)
    valid = ~np.isnan(stitched_raw).any(axis=1)

    metrics = {}
    for i, app in enumerate(APPLIANCES):
        metrics[app] = calculate_nilm_metrics(
            true_raw[valid, i],
            stitched_raw[valid, i],
            threshold=THRESHOLDS[app],
        )
    return metrics, stitched_raw, valid
...
def train_model(data_dict, save_dir, hidden_size=64, dt=0.1,
    ...
    print(f"seq2seq=True  stitched_metrics=True  lambda_phys={lambda_phys}  epsilon={epsilon_w} W")
    ...
    for epoch in range(EPOCHS):
        ...
        per_app_metrics, _, _ = compute_stitched_metrics(
            data_dict["val"], y_pred_all, y_scalers
        )
    ...
    test_metrics, stitched_test_raw, stitched_valid = compute_stitched_metrics(
        data_dict["test"], y_pred_te, y_scalers
    )
    ...
    plot_stitched_test_trace(
        data_dict["test"], stitched_test_raw, stitched_valid, save_dir
    )
...
def plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "pinn_lnn_ukdale_v4_loss.png"), dpi=150, bbox_inches="tight")
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2, figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle("PINN-LNN UKDALE v4 seq2seq - Stitched Val Metrics", fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "pinn_lnn_ukdale_v4_per_appliance.png"), dpi=150, bbox_inches="tight")
    plt.close()


def plot_stitched_test_trace(test_df, stitched_raw, valid_mask, save_dir):
    x_hours = np.arange(len(test_df), dtype=np.float32) * 6.0 / 3600.0

    fig, axes = plt.subplots(len(APPLIANCES) + 1, 1, figsize=(16, 12), sharex=True)
    fig.suptitle("PINN-LNN UKDALE v4 - Stitched Test Trace", fontsize=13)
    ...
    for i, app in enumerate(APPLIANCES):
        ...
        ax.plot(
            x_hours[valid_mask],
            stitched_raw[valid_mask, i],
            color="tomato",
            linestyle="--",
            linewidth=0.8,
            label=f"stitched pred {app}",
        )
        ax.axhline(THRESHOLDS[app], color="gray", linewidth=0.6, linestyle=":")
        ax.set_title(app)
        ax.set_ylabel("W")
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, "pinn_lnn_ukdale_v4_stitched_test_trace.png"),
        dpi=150,
        bbox_inches="tight",
    )
    plt.close()
...
def save_results(save_dir, test_metrics, hidden_size, dt, lambda_phys, epsilon_w):
    config = {
        ...
        "model": "Seq2SeqPhysicsInformedLiquidNetworkModelV4",
        "description": "full-window seq2seq PINN-LNN, BCE disabled, stitched timeline metrics",
        "window_size": WIN,
        "stride": STRIDE,
    ...
    with open(os.path.join(save_dir, "pinn_lnn_ukdale_v4_results.json"), "w", encoding="utf-8") as f:
        json.dump(config, f, indent=4)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train v4 seq2seq PINN-LNN with stitched timeline metrics."
    )
...
def main():
    ...
    save_dir = f"models/pinn_lnn_ukdale_v4_{timestamp}"
