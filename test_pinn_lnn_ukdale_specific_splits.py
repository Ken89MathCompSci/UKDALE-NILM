...
Standard LNN scripts train one appliance independently. This script trains a
SINGLE multi-output model that predicts ALL appliances simultaneously, then
adds a Physical Consistency Loss that penalises the model whenever the sum of
predicted appliance loads exceeds the measured aggregate:

    L_total = L_MSE + λ * L_phys

    L_MSE   = mean over appliances of MSE(p_hat_i, p_true_i)
    L_phys  = mean(ReLU(Σ p_hat_i_raw - P_agg_raw - ε))

    where:
        p_hat_i_raw  — predicted appliance power in raw Watts (inverse-scaled)
        P_agg_raw    — measured aggregate at the midpoint timestep in raw Watts
        ε            — tolerance for unlabelled background loads (default 50 W)
        λ            — physics regularisation weight (default 0.1)

Why one-sided (ReLU) rather than absolute-value?
    The four appliances do NOT cover all loads in the home. A strict two-sided
    constraint would force the model to invent extra power to fill the gap.
    The one-sided version only punishes physically impossible over-assignment.
...
class PhysicsInformedLiquidNetworkModel(nn.Module):
    ...
    The shared encoder sees the same mains window for all appliances.
    Each head specialises to its appliance. The physics constraint is applied
    to the concatenated output (batch, n_appliances) at training time.
    """
    ...
    def __init__(self, input_size, hidden_size, n_appliances, dt=0.1):
        ...
        # Normalise shared hidden state before heads
    ...
    def forward(self, x):
        ...
        Returns:
            out: (batch, n_appliances) — one prediction per appliance
        """
        ...
        h = self.norm(h)
        # Concatenate all heads: [(batch,1), ...] → (batch, n_appliances)
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        'dataset': 'UKDALE',
        'model': 'PhysicsInformedLiquidNetworkModel',
        'description': 'shared AdvancedLiquidTimeLayer encoder + per-appliance heads + L_phys',
...
if __name__ == "__main__":
    ...
    save_dir     = f"models/pinn_lnn_ukdale_{timestamp}"
