"""
GRU baseline for NILM -- APR-new-House5-dataset/ splits.
...
3. PER-APPLIANCE THRESHOLDS (not the uniform 10 W used for House 1) --
   matches generate_apr_new_house5_dataset.py / analyze_apr_new_house5_distributions.py,
   using building5.yaml's own per-appliance on_power_threshold values:
       dishwasher      : 10 W (clean channel, off = 0 W)
       fridge          : 50 W (clean channel, off = 0 W; building5.yaml
                         on_power_threshold for fridge_freezer)
       microwave       : 200 W (channel never reads 0 W -- persistent
                         standby floor ~48-52 W; a 10 W threshold would
                         read 100% ON for the entire split)
       washing_machine : 20 W (channel never reads 0 W -- persistent
                         standby floor ~14-17 W; a 10 W threshold would
                         read 100% ON for the entire split)
...
THRESHOLDS  = {
    'dishwasher':      10.0,
    'fridge':          50.0,   # building5.yaml on_power_threshold (clean channel)
    'microwave':      200.0,   # standby floor ~48-52 W; building5.yaml on_power_threshold
    'washing_machine': 20.0,   # standby floor ~14-17 W; building5.yaml on_power_threshold
}
...
def load_data(dataset_dir=DATASET_DIR):
    """Load train / validation / test from APR-new-House5-dataset/ CSV files."""
...
def train_gru_on_appliance(data_dict, appliance_name, window_size=100,
                            ...
                            save_dir='models/gru_apr_new_house5_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_apr_new_house5_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"gru_apr_new_house5_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'gru_apr_new_house5_{appliance_name}_history.json'),
...
def test_gru_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"gru_apr_new_house5_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"gru_apr_new_house5_{appliance_name}_best.pth"),
    ...
    print(f"\nGRU APR-new-House5-dataset testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing GRU on APR-new-House5-dataset...")
    ...
    print(f"\nSummary of GRU testing on APR-new-House5-dataset:")
