"""
test_resnet_apr_new_house5_dataset_v2.py
==========================================
House 5 counterpart to test_resnet_apr_new_house1_dataset_v2.py and
test_resnet_apr_new_house2_dataset_v2.py -- ported directly to the "v2"
gated-head architecture (there is no separate un-gated House 5 ResNet
script in this repo; test_gru/lnn/lstm_apr_new_house5_dataset.py serve as
this house's un-gated MSE-only baselines).

GATE HEAD
    ResNetModel's usual single unbounded nn.Linear(base_width*4, output_size)
    head, trained with MSE only, is replaced with two heads sharing the same
    conv/residual trunk: a gate head predicting P(ON) via BCEWithLogits, and
    a power head predicting a sigmoid-bounded power estimate.
...
matches the House 1/House 2 v2 scripts unchanged.
"""
...
class GatedResNetModel(nn.Module):
    ...
    GatedResNetModel, adapted to House 5's pos_weight clamp.
    """
...
def train_resnet_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                               ...
                               save_dir='models/resnet_apr_new_house5_dataset_v2'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_apr_new_house5_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_apr_new_house5_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'resnet_apr_new_house5_v2_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_apr_new_house5_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_apr_new_house5_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated ResNet v2 APR-new-House5-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated ResNet v2 on APR-new-House5-dataset...")
    ...
    print(f"\nSummary of Gated ResNet v2 testing on APR-new-House5-dataset:")
