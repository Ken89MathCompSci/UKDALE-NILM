"""
combined_pinn_lnn_apr_new_house5_dataset.py
=============================================
Dual-head PINN + Gated-Detection LNN for NILM -- APR-new-House5-dataset/ splits.
...
analyze_apr_new_house5_distributions.py) found dishwasher segments running
up to 47 min (p95) and fridge medians around 20-25 min, both of which a
shorter WIN=100/200 window could clip mid-cycle.
"""
...
DEFAULT_DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-new-House5-dataset')
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading APR-new-House5-dataset CSV data from '{dataset_dir}' ...")
...
def train(data_dict: dict, save_dir: str,
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house5_dataset_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-new-House5-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house5_dataset_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-new-House5-dataset) -- Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house5_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_apr_new_house5_dataset_{timestamp}'
    )
