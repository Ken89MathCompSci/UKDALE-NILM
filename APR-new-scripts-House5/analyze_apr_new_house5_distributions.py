"""
On-duration and time-of-day distribution analysis for APR-new-House5-dataset/,
...
generate_apr_new_house5_dataset.py):
    microwave        -- never reads 0 W; standby floor ~48-52 W. A 10 W
                        threshold reads 100% ON. building5.yaml's own
                        on_power_threshold (200 W) is used here instead.
    washing_machine  -- never reads 0 W; standby floor ~14-17 W. A 10 W
                        threshold reads 100% ON. building5.yaml's
                        on_power_threshold (20 W) is used here instead.
    fridge           -- clean channel (0 W when off); building5.yaml's
                        on_power_threshold (50 W) is used for reporting.
    dishwasher       -- clean channel (0 W when off); same behaviour as
                        Houses 1/2, 10 W threshold.

Per split, per appliance:
    1. Threshold (see THRESHOLDS dict) to get a binary ON/OFF state per 6s sample.
    2. Run-length-encode contiguous ON segments (vectorized).
...
Outputs (written into APR-new-House5-dataset/):
...
THRESHOLDS   = {
    'dishwasher':      10.0,
    'fridge':          50.0,   # standby 0 W, compressor cycles; on_power_threshold from building5.yaml
    'microwave':      200.0,   # standby ~48-52 W; on_power_threshold from building5.yaml
    'washing_machine': 20.0,   # standby ~14-17 W; on_power_threshold from building5.yaml
}
...
def load_splits(dataset_dir=DATASET_DIR):
    """Load train/validation/test as separate DataFrames."""
    dfs = {}
    for name in SPLITS:
...
def plot_split(results, split_name, out_dir=DATASET_DIR):
    ...
    fig.suptitle(f'APR-new-House5-dataset -- ON-Duration & Time-of-Day '
...
def plot_comparison(all_results, out_dir=DATASET_DIR):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2, figsize=(14, 4 * len(APPLIANCES)))
    fig.suptitle('APR-new-House5-dataset -- Train vs Validation vs Test comparison',
                 fontsize=13)
