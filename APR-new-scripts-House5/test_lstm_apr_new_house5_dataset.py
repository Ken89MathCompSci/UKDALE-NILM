"""
LSTM baseline for NILM -- APR-new-House5-dataset/ splits.
...
same three changes described in test_gru_apr_new_house5_dataset.py:
  ...
  3. Per-appliance thresholds (not the uniform 10 W used for House 1) --
     ...
     generate_apr_new_house5_dataset.py / analyze_apr_new_house5_distributions.py:
       dishwasher=10 W (clean channel, off = 0 W), fridge=50 W (clean channel),
       microwave=200 W (channel never reads 0 W -- persistent standby floor
       ~48-52 W, so a 10 W threshold would read 100% ON for the whole split),
       washing_machine=20 W (channel never reads 0 W -- persistent standby
       floor ~14-17 W, same issue at 10 W).

Also restores the best (lowest val loss) checkpoint before test evaluation --
the old LSTM script evaluated on whatever state the model was in when the
training loop exited, not necessarily the best epoch.

Architecture/training loop otherwise unchanged: one LSTM model per appliance
...
def train_lstm_on_appliance(data_dict, appliance_name, window_size=100,
                             ...
                             save_dir='models/lstm_apr_new_house5_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"lstm_apr_new_house5_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"lstm_apr_new_house5_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'lstm_apr_new_house5_{appliance_name}_history.json'),
...
def test_lstm_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"lstm_apr_new_house5_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"lstm_apr_new_house5_{appliance_name}_best.pth"),
    ...
    print(f"\nLSTM APR-new-House5-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing LSTM on APR-new-House5-dataset...")
    ...
    print(f"\nSummary of LSTM testing on APR-new-House5-dataset:")
