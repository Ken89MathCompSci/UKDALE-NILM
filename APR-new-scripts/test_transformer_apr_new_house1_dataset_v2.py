"""
test_transformer_apr_new_house1_dataset_v2.py
================================================
v2 of test_transformer_apr_new_house1_dataset.py -- two targeted fixes for
the precision collapse seen in the v1 results:

    Appliance            F1  Precision   Recall
    dishwasher       0.0273     0.0141   0.4581   <- ~70 FP per TP
    fridge           0.7008     0.5432   0.9870
    microwave        0.3713     0.2301   0.9609
    washing_machine  0.6707     0.5130   0.9686

Recall is high-to-near-perfect everywhere; precision collapses hardest on
the rarest classes (dishwasher, microwave). This is the same "recall~1.0,
precision near 0" pattern that combined_pinn_lnn_apr_new_house1_dataset.py's
own docstring names as the reason its gate mechanism exists -- and this
Transformer, an unrelated architecture family with no gate at all,
reproduces it, which suggests the cause is generic to plain MSE-only
regression on an imbalanced NILM target, not specific to any one model.

FIX 1 -- Gate head (main fix)
    v1's SimpleTransformerModel has a single unbounded linear output,
    trained with MSE only -- nothing in the loss tells it to commit to
    exactly 0 during OFF periods, so its regression output drifts above
    the threshold often enough to flood precision with false positives,
    worst on the rarest appliances (dishwasher ~3% ON, microwave ~5% ON).
    Fix: add a second head that predicts P(ON) via BCEWithLogits (with
    pos_weight from the training ON-fraction, same formula used in
    combined_pinn_lnn_apr_new_house1_dataset.py's GatedEventLoss), and
    multiply it into a sigmoid-bounded power head:
        gated_power = sigmoid(gate_logit) * sigmoid(power_logit)
    Loss = MSE(gated_power, y) + lambda_event * BCEWithLogits(gate_logit, y_on).
    When the gate is confidently OFF, gated_power is pulled toward 0
    regardless of what the power head alone would have said -- the same
    mechanism that fixed this exact failure mode for the LNN architecture.
...
FIX 2 -- Adaptive per-appliance thresholds (train-locked)
    ...
    washing_machine -- 10W is only correct for fridge. Using it for
    dishwasher in particular means both training (the gate BCE target)
    and evaluation (precision/recall) are computed against a boundary
    that's 11W too low, which will itself generate spurious "ON" ground
    truth and predictions right at the noise floor.
    ...
    split only, reused for val and test -- both for the gate's BCE target
    and for the reported precision/recall/F1.
...
class GatedTransformerModel(nn.Module):
    ...
    combined_pinn_lnn_apr_new_house1_dataset.py, adapted to seq2point
    (one prediction per window instead of one per timestep).
    """
...
def train_transformer_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                                    ...
                                    save_dir='models/transformer_apr_new_house1_dataset_v2'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"transformer_apr_new_house1_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"transformer_apr_new_house1_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'transformer_apr_new_house1_v2_{appliance_name}_history.json'),
...
def test_transformer_on_all_appliances(window_size=100, hidden_size=128, num_layers=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"transformer_apr_new_house1_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"transformer_apr_new_house1_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated Transformer v2 APR-new-House1-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated Transformer v2 on APR-new-House1-dataset...")
    ...
    print(f"\nSummary of Gated Transformer v2 testing on APR-new-House1-dataset:")
