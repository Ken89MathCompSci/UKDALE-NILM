"""
Physics-Informed Basic LNN (PINN-BasicLNN) for NILM — dataset/ splits.

Derived from test_pinn_basic_lnn_ukdale_specific_splits.py with two changes:

1. DATA SOURCE — reads directly from dataset/ CSV files:
       dataset/UKDALE_HF_train.csv      (House 1, 2014-11-09)
       dataset/UKDALE_HF_validation.csv (House 1, 2014-12-07)
       dataset/UKDALE_HF_test.csv       (House 5, 2014-08-24)
   Columns: timestamp, aggregate, dishwasher, fridge, microwave, washing_machine

2. RAISED ON/OFF THRESHOLDS — House 5 (test) microwave and washing_machine
   have persistent standby draws that never reach 0 W:
       microwave:       10 W  ->  30 W   (H5 floor is ~25 W)
       washing_machine:  0.5 W -> 18 W   (H5 floor is ~14 W)
   The higher thresholds clear the standby artefact while remaining well
   below real activation peaks (microwave >200 W, washer >200 W).
...
# Raised thresholds: microwave and washing_machine cleared of H5 standby floor.
THRESHOLDS = {
    'dishwasher':      10.0,
    'fridge':          10.0,
    'microwave':       30.0,   # raised from 10 W — H5 standby floor ~25 W
    'washing_machine': 18.0,   # raised from  0.5 W — H5 standby floor ~14 W
}
...
# Path to dataset/ relative to this script's parent directory
...
def load_data(dataset_dir=DATASET_DIR):
    """Load train / validation / test from dataset/ CSV files."""
    print(f"Loading data from: {os.path.abspath(dataset_dir)}")

    def _load(filename):
        path = os.path.join(dataset_dir, filename)
        df = pd.read_csv(path, index_col='timestamp', parse_dates=True)
        # Rename aggregate -> main to share create_sequences logic
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        'dataset': 'UKDALE dataset/ (H1 train/val, H5 test)',
        'model': 'PhysicsInformedBasicLiquidNetworkModel',
        'description': 'basic LNN (fixed tau, no gate) + L_phys; raised thresholds for H5 standby',
    ...
    with open(os.path.join(save_dir, 'pinn_basic_lnn_dataset_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_dataset_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-BasicLNN dataset/ — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_basic_lnn_dataset_{timestamp}"
    )
