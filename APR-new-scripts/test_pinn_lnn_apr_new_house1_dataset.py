"""
Physics-Informed LNN (PINN-LNN) for NILM — APR-new-House1-dataset splits.
...
def train_pinn_model(data_dict, save_dir,
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_apr_new_house1_dataset_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_apr_new_house1_dataset_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN APR-new-House1-dataset -- Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_apr_new_house1_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_lnn_apr_new_house1_dataset_{timestamp}"
    )
