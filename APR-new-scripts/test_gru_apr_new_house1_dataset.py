"""
GRU baseline for NILM -- APR-new-House1-dataset/ splits.
...
1. DATA SOURCE -- reads directly from APR-new-House1-dataset/ CSV files instead
   ...
   Appliances: dishwasher, fridge, microwave, washing_machine (all House 1 -- no
   cross-house domain shift, no standby artefact).
...
3. UNIFORM 10 W THRESHOLD -- all appliances use 10 W (the old script special-
   cased 'washer dryer' to 0.5, tuned for a different pickled data scale that
   doesn't apply here). Matches test_pinn_lnn_apr_dataset.py's convention for
   this repo's House-1-only datasets.
...
def train_gru_on_appliance(data_dict, appliance_name, window_size=100,
                            ...
                            save_dir='models/gru_apr_new_house1_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_apr_new_house1_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"gru_apr_new_house1_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'gru_apr_new_house1_{appliance_name}_history.json'),
...
def test_gru_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"gru_apr_new_house1_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"gru_apr_new_house1_{appliance_name}_best.pth"),
    ...
    print(f"\nGRU APR-new-House1-dataset testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing GRU on APR-new-House1-dataset...")
    ...
    print(f"\nSummary of GRU testing on APR-new-House1-dataset:")
