"""
test_resnet_apr_new_house1_dataset_v2.py
==========================================
v2 of test_resnet_apr_new_house1_dataset.py -- two targeted fixes for the
precision collapse seen in the v1 results:

    Appliance            F1  Precision   Recall
    dishwasher       0.2191     0.1653   0.3246
    fridge           0.7008     0.5414   0.9933
    microwave        0.3128     0.1879   0.9336
    washing_machine  0.8639     0.7686   0.9862

Recall is high-to-near-perfect on three of the four appliances (fridge,
microwave, washing_machine all >0.93); precision lags well behind
everywhere and collapses hardest on the rarest classes (dishwasher,
microwave). This is the same "recall high, precision low" pattern already
diagnosed for test_transformer_apr_new_house1_dataset.py -- and the same
root cause: a plain MSE-only seq2point regressor with no explicit
detection mechanism, evaluated against a threshold that's wrong for three
of the four appliances.

FIX 1 -- Gate head (main fix)
    v1's ResNetModel ends in a single unbounded nn.Linear(base_width*4,
    output_size), trained with MSE only -- nothing in the loss tells it to
    commit to exactly 0 during OFF periods, so its regression output drifts
    above the threshold often enough to flood precision with false
    positives, worst on the rarest appliances (dishwasher ~3% ON, microwave
    ~5% ON). Fix: replace the single fc head with two heads sharing the
    same conv/residual trunk, and predict P(ON) via BCEWithLogits (with
    pos_weight from the training ON-fraction, same formula used in
    combined_pinn_lnn_apr_new_house1_dataset.py's GatedEventLoss):
        gated_power = sigmoid(gate_logit) * sigmoid(power_logit)
    Loss = MSE(gated_power, y) + lambda_event * BCEWithLogits(gate_logit, y_on).
    Same mechanism that fixes this exact failure mode for the LNN
    architecture and for the Gated Transformer v2.

FIX 2 -- Adaptive per-appliance thresholds (train-locked)
    v1 used a flat 10W threshold for all four appliances. House 1's own
    adaptive thresholds (same formula used throughout this repo) are
    21W / 10W / 21W / 23W for dishwasher / fridge / microwave /
    washing_machine -- 10W is only correct for fridge. Using it elsewhere
    means both training (the gate BCE target) and evaluation (precision/
    recall) are computed against a boundary that's off by up to 13W.
    Fix: compute_adaptive_thresholds() (identical formula to the
    combined_pinn_lnn_apr_new_house1_dataset*.py scripts) on the training
    split only, reused for val and test.
...
val loss, checkpoint restore before test, base_width=32/layers=[2,2,2] --
...
# ---------------------------------------------------------------------------
# FIX 1: gated ResNet -- same conv/residual trunk as v1's ResNetModel, but
# with a second (gate) head. gated_power = sigmoid(gate) * sigmoid(power).
# ---------------------------------------------------------------------------

class GatedResNetModel(nn.Module):
    """
    Same conv/residual trunk as models.ResNetModel (initial conv+bn+relu+
    maxpool, 3 residual stages via ResidualBlock, global average pool).
    Output head is replaced with two heads:
    ...
    This mirrors CombinedPINNAdvancedLNN's dual-head design in
    combined_pinn_lnn_apr_new_house1_dataset.py and GatedTransformerModel
    in test_transformer_apr_new_house1_dataset_v2.py, adapted to the
    ResNet backbone.
    """
    ...
    def __init__(self, input_size, output_size, layers=[2, 2, 2], base_width=32):
        ...
        self.gate_head  = nn.Linear(base_width*4, output_size)
        self.power_head = nn.Linear(base_width*4, output_size)
...
def train_resnet_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                               base_width=32, layers=None,
                               epochs=80, lr=0.001, patience=20, lambda_event=LAMBDA_EVENT,
                               save_dir='models/resnet_apr_new_house1_dataset_v2'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    # formula as GatedEventLoss in combined_pinn_lnn_apr_new_house1_dataset.py
    ...
    print(f"Starting Gated ResNet training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_apr_new_house1_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_apr_new_house1_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'resnet_apr_new_house1_v2_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_apr_new_house1_dataset_v2_{timestamp}")

    all_results = {}

    for appliance_name in APPLIANCES:
        print(f"\n{'='*60}")
        print(f"Testing Gated ResNet on {appliance_name}")
        print(f"{'='*60}\n")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_apr_new_house1_v2_{appliance_name}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"Successfully tested Gated ResNet on {appliance_name}")
    ...
    print(f"\nGated ResNet v2 APR-new-House1-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated ResNet v2 on APR-new-House1-dataset...")
    ...
    print(f"\nSummary of Gated ResNet v2 testing on APR-new-House1-dataset:")
