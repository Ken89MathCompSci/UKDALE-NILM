"""
Physics-Informed Basic LNN (PINN-BasicLNN) for NILM -- APR-House1-House2-dataset/ splits.

Derived from test_pinn_basic_lnn_dataset_dir.py with two changes:

1. DATA SOURCE -- reads from APR-House1-House2-dataset/ CSV files:
       APR-House1-House2-dataset/UKDALE_HF_train.csv      (House 2, 2013-06-15)
       APR-House1-House2-dataset/UKDALE_HF_validation.csv (House 2, 2013-09-01)
       APR-House1-House2-dataset/UKDALE_HF_test.csv       (House 1, 2014-11-09)
   Columns: timestamp, aggregate, dishwasher, fridge, microwave, washing_machine

2. THRESHOLDS -- adjusted for House 2 data characteristics:
       fridge:          10 W -> 15 W   (H2 fridge has a ~10 W standby floor)
       microwave:       30 W -> 10 W   (H2/H1 microwave have no persistent standby)
       washing_machine: 18 W -> 10 W   (H2/H1 washing_machine have no persistent standby)
   The test split is House 1 (clean ON/OFF), so 10 W is valid for all appliances there.

Architecture and training loop are identical to the source script:
    BasicLiquidTimeLayer (fixed tau, no gate) -> LayerNorm -> per-appliance heads
    Stage 1 (epochs 1-WARMUP_EPOCHS): MSE only
    Stage 2:                           MSE + lambda*L_phys + weighted BCE
"""
...
# Fridge raised to 15 W: H2 fridge has a ~10 W standby floor that would
# otherwise mark it as ON ~75-85% of the time in train/val.
THRESHOLDS = {
    'dishwasher':      10.0,
    'fridge':          15.0,   # raised from 10 W -- H2 fridge standby floor ~10 W
    'microwave':       10.0,   # reverted from 30 W -- no H2/H1 standby issue
    'washing_machine': 10.0,   # reverted from 18 W -- no H2/H1 standby issue
}
...
DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-House1-House2-dataset')
...
def load_data(dataset_dir=DATASET_DIR):
    """Load train / validation / test from APR-House1-House2-dataset/ CSV files."""
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        'dataset': 'APR-House1-House2-dataset (H2 train Jun-15 / val Sep-01, H1 test Nov-09)',
        'model': 'PhysicsInformedBasicLiquidNetworkModel',
        'description': 'basic LNN (fixed tau, no gate) + L_phys; fridge threshold raised for H2 standby floor',
    ...
    with open(os.path.join(save_dir, 'pinn_basic_lnn_h1h2_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_h1h2_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-BasicLNN APR-House1-House2-dataset -- Per-Appliance Val Metrics',
                 fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_h1h2_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_basic_lnn_h1h2_{timestamp}"
    )
