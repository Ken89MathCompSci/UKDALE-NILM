"""
Simplified Transformer baseline for NILM -- APR-new-House1-dataset/ splits.

Derived from Old-LNN_Algorithms/test_transformer_ukdale_specific_splits.py with
the same three changes described in test_gru_apr_new_house1_dataset.py:
...
def train_transformer_on_appliance(data_dict, appliance_name, window_size=100,
                                    ...
                                    save_dir='models/transformer_apr_new_house1_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"transformer_apr_new_house1_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"transformer_apr_new_house1_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'transformer_apr_new_house1_{appliance_name}_history.json'),
...
def test_transformer_on_all_appliances(window_size=100, hidden_size=128, num_layers=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"transformer_apr_new_house1_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"transformer_apr_new_house1_{appliance_name}_best.pth"),
    ...
    print(f"\nTransformer APR-new-House1-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Transformer on APR-new-House1-dataset...")
    ...
    print(f"\nSummary of Transformer testing on APR-new-House1-dataset:")
