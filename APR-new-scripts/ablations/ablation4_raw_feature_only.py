"""
ablation4_raw_feature_only.py
===============================
Tier 1 ablation #4 of combined_pinn_lnn_apr_new_house1_dataset.py: feed the
...
thresholds, checkpoint-on-val-MSE, WIN=299 are all otherwise UNCHANGED
from the baseline, so this isolates exactly one variable: whether the
hand-engineered channels (median/EMA smoothing, residual, deltas, rolling
stats) are earning their keep over letting the CNN learn those
transformations itself from raw power.
...
# ---------------------------------------------------------------------------
# 1-channel feature extraction  -- ABLATED (baseline had 8 channels)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Combined model  (identical to baseline -- only its input channel count changes)
# ---------------------------------------------------------------------------

class CombinedPINNAdvancedLNN(nn.Module):
    """
    Same architecture as the baseline's CombinedPINNAdvancedLNN. Only
...
def train(data_dict: dict, save_dir: str,
    ...
    print(f"  n_feat = {n_feat}  (baseline uses 8; this ablation uses 1)")
    ...
    cfg = {
        ...
        'ablation_description': 'compute_features() returns raw aggregate power only '
                                '(1 channel) instead of the baseline 8-channel stack '
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('Ablation 4: Raw Feature Only (APR-new-House1-dataset) -- Loss Curves')
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('Ablation 4: Raw Feature Only (APR-new-House1-dataset) -- Per-Appliance Validation Metrics')
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'ablation4_raw_feature_only_{timestamp}'
    )
