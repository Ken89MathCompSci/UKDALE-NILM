"""
ablation1_no_gate.py
=====================
Tier 1 ablation #1 of combined_pinn_lnn_apr_new_house1_dataset.py: remove
...
features, physics loss, thresholds, checkpoint-on-val-MSE, WIN=299 -- is
UNCHANGED from the baseline, so this isolates exactly one variable: the
gate mechanism.
...
Compare test_metrics from this script against the baseline's saved results
(models/combined_pinn_lnn_apr_new_house1_dataset_*/..._results.json) --
...
class UngatedPINNLNN(nn.Module):
    ...
    Same CNN front-end and bidirectional LiquidCell as the baseline; only
...
# ---------------------------------------------------------------------------
# Losses  (physics loss identical to baseline; GatedEventLoss removed)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Training  (loss = MSE (+ physics after warmup); no gate-event term)
# ---------------------------------------------------------------------------
...
def train(data_dict: dict, save_dir: str,
    ...
    print(f"Warmup: MSE-only for {WARMUP_EPOCHS} epochs, then + physics (no gate-event term).\n")
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in pbar:
            ...
            loss   = l_mse if epoch < WARMUP_EPOCHS else (l_mse + lambda_phys * l_phys)
            loss.backward()
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('Ablation 1: No Gate (APR-new-House1-dataset) -- Loss Curves')
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('Ablation 1: No Gate (APR-new-House1-dataset) -- Per-Appliance Validation Metrics')
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'ablation1_no_gate_{timestamp}'
    )
