"""
ablation3_gru_instead_of_liquidcell.py
========================================
Tier 1 ablation #3 of combined_pinn_lnn_apr_new_house1_dataset.py: swap the
...
checkpoint-on-val-MSE, WIN=299 -- is UNCHANGED from the baseline, so this
isolates exactly one variable: whether the "liquid" (ODE-style,
input-modulated tau) recurrence is actually earning its complexity over a
standard, cheaper GRU cell.
...
# ---------------------------------------------------------------------------
# Constants  (identical to baseline)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Adaptive thresholds  (identical to baseline)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8-channel feature extraction  (identical to baseline)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Sequence creation  (identical to baseline)
# ---------------------------------------------------------------------------
...
class CombinedPINNGRU(nn.Module):
    ...
    bidirectional structure, gate mechanism -- is unchanged.
    """
...
# ---------------------------------------------------------------------------
# Overlap-average reconstruction  (identical to baseline)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Training  (identical loss schedule to baseline)
# ---------------------------------------------------------------------------
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('Ablation 3: GRUCell instead of LiquidCell (APR-new-House1-dataset) -- Loss Curves')
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('Ablation 3: GRUCell instead of LiquidCell (APR-new-House1-dataset) -- Per-Appliance Validation Metrics')
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'ablation3_gru_instead_of_liquidcell_{timestamp}'
    )
