"""
ablation2_unidirectional_lnn.py
=================================
Tier 1 ablation #2 of combined_pinn_lnn_apr_new_house1_dataset.py: drop the
...
isolates exactly one variable: whether future context (the backward pass)
matters.
...
# ---------------------------------------------------------------------------
# Model components  (CNN + LiquidCell identical to baseline)
# ---------------------------------------------------------------------------
...
class UnidirectionalPINNLNN(nn.Module):
    ...
    the baseline -- only the recurrent context is halved.
    """
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('Ablation 2: Unidirectional LNN (APR-new-House1-dataset) -- Loss Curves')
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('Ablation 2: Unidirectional LNN (APR-new-House1-dataset) -- Per-Appliance Validation Metrics')
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'ablation2_unidirectional_lnn_{timestamp}'
    )
