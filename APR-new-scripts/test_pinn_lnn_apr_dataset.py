"""
Physics-Informed LNN (PINN-LNN) for NILM — APR-dataset splits.

Derived from test_pinn_lnn_ukdale_specific_splits.py with two changes:

1. DATA SOURCE — reads directly from APR-dataset/ CSV files:
       APR-dataset/UKDALE_HF_train.csv      (House 1, 2014-11-09)
       APR-dataset/UKDALE_HF_validation.csv (House 1, 2014-12-07)
       APR-dataset/UKDALE_HF_test.csv       (House 1, 2014-12-19)
...
2. UNIFORM 10 W THRESHOLD — all appliances use 10 W as the ON/OFF boundary.
   With House 1 test data there is no persistent standby floor, so the standard
   threshold is valid for every appliance including microwave and washing_machine.
...
# Uniform 10 W threshold — valid for all appliances with House 1 test data.
THRESHOLDS = {
    'dishwasher':      10.0,
    'fridge':          10.0,
    'microwave':       10.0,
...
DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-dataset')
...
def load_data(dataset_dir=DATASET_DIR):
    """Load train / validation / test from APR-dataset/ CSV files."""
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        'dataset': 'APR-dataset (H1 train Nov-09 / val Dec-07 / test Dec-19)',
        'model': 'PhysicsInformedLiquidNetworkModel',
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_apr_dataset_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_apr_dataset_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN APR-dataset -- Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_apr_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_lnn_apr_dataset_{timestamp}"
    )
