"""
test_tcn_apr_new_house1_dataset_v2.py
=======================================
v2 of test_tcn_apr_new_house1_dataset.py -- same two fixes already applied
to the Transformer and ResNet House 1 v2 scripts, this time to TCNModel.

v1 test results:

    Appliance            F1  Precision   Recall      MAE      SAE
    dishwasher       0.2860     0.2440   0.3455    18.24  13.7784
    fridge           0.6810     0.5164   0.9995    40.94  22.1704
    microwave        0.2167     0.1237   0.8750    23.77  12.0180
    washing_machine  0.7602     0.6258   0.9680    28.11  10.4684

Same signature as the Transformer/ResNet House 1 results: recall is
high-to-near-perfect everywhere (fridge R=0.9995!), precision collapses
hardest on the rarest classes (dishwasher, microwave). Same two root
causes, confirmed already for this exact dataset/threshold combination in
test_transformer_apr_new_house1_dataset_v2.py and
test_resnet_apr_new_house1_dataset_v2.py:

FIX 1 -- Gate head (main fix)
    v1's TCNModel ends in a single unbounded nn.Linear(num_channels[-1],
    output_size) after global-average-pooling the TCN stack's output,
    trained with MSE only -- nothing in the loss tells it to commit to
    exactly 0 during OFF periods. Replace the single fc head with two
    heads sharing the same dilated-TCN trunk, and predict P(ON) via
    BCEWithLogits (pos_weight from the training ON-fraction, same formula
    used in combined_pinn_lnn_apr_new_house1_dataset.py's GatedEventLoss):
...
FIX 2 -- Adaptive per-appliance thresholds (train-locked)
    ...
    washing_machine -- 10W is only correct for fridge. Fix:
    compute_adaptive_thresholds() (identical formula to the
...
restore before test -- is unchanged from v1.
"""
...
class GatedTCNModel(nn.Module):
    ...
    combined_pinn_lnn_apr_new_house1_dataset.py and the Gated Transformer/
    ResNet v2 scripts, adapted to the TCN backbone.
    """
...
def train_tcn_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                            ...
                            save_dir='models/tcn_apr_new_house1_dataset_v2'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"tcn_apr_new_house1_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_apr_new_house1_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'tcn_apr_new_house1_v2_{appliance_name}_history.json'),
...
def test_tcn_on_all_appliances(window_size=100, num_channels=None, kernel_size=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"tcn_apr_new_house1_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"tcn_apr_new_house1_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated TCN v2 APR-new-House1-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated TCN v2 on APR-new-House1-dataset...")
    ...
    print(f"\nSummary of Gated TCN v2 testing on APR-new-House1-dataset:")
