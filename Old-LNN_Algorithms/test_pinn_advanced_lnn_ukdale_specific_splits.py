"""
Physics-Informed Advanced LNN (PINN-AdvancedLNN) for NILM — UKDALE specific splits.

Upgrade over test_pinn_lnn_ukdale_specific_splits.py:
    The shared encoder is replaced with a stacked AdvancedLiquidNetworkModel
    (num_layers=2) — each layer has adaptive tau and an input-dependent gate,
    with inter-layer LayerNorm between stacked layers.
...
def train_pinn_model(data_dict, save_dir,
    ...
    print(f"λ_phys={lambda_phys}  ε={epsilon_w} W  "
    ...
    config = {
        ...
        'description': (
            ...
            f'inter-layer LayerNorm) + per-appliance heads + L_phys'
        ),
    ...
    with open(os.path.join(save_dir, 'pinn_advanced_lnn_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_advanced_lnn_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-AdvancedLNN UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_advanced_lnn_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_advanced_lnn_ukdale_{timestamp}"
