...
def train_lstm_on_appliance(data_dict, appliance_name, window_size=100,
                             ...
                             save_dir='models/lstm_ukdale_specific'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"lstm_ukdale_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"lstm_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'lstm_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_lstm_on_all_ukdale_appliances(window_size=100, hidden_size=128, num_layers=2,
                                       bidirectional=True, epochs=80, lr=0.001, patience=20):

    data_dict = load_ukdale_specific_splits()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/lstm_ukdale_specific_test_{timestamp}"
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"lstm_ukdale_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    print(f"\nLSTM UKDALE testing completed. Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing LSTM on UKDALE dataset with specific splits...")
    ...
    results = test_lstm_on_all_ukdale_appliances(
        window_size=100, hidden_size=128, num_layers=2,
        bidirectional=True, epochs=80, lr=0.001, patience=20)

    print(f"\nSummary of LSTM testing on UKDALE dataset:")
