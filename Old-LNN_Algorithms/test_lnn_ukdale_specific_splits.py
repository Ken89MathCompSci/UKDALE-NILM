...
def train_on_appliance(data_dict, appliance_name, window_size=100,
                       ...
                       save_dir='models/lnn_ukdale_specific'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"lnn_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'lnn_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_on_all_appliances(window_size=100, hidden_size=64, dt=0.1,
    ...
    base_save_dir = f"models/lnn_ukdale_specific_test_{timestamp}"
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    print(f"\nLiquidNetworkModel UKDALE testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing LiquidNetworkModel on UKDALE dataset with specific splits...")
    ...
    print(f"\nSummary of LiquidNetworkModel testing on UKDALE dataset:")
