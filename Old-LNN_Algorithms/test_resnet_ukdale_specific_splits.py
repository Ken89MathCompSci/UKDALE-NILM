...
def train_resnet_on_appliance(data_dict, appliance_name, window_size=100,
                               ...
                               save_dir='models/resnet_ukdale_specific'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_ukdale_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"resnet_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'resnet_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_resnet_on_all_ukdale_appliances(window_size=100, base_width=32, layers=None,
                                         epochs=80, lr=0.001, patience=20):
    if layers is None:
        layers = [2, 2, 2]
    ...
    base_save_dir = f"models/resnet_ukdale_specific_test_{timestamp}"
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"resnet_ukdale_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    print(f"\nResNet UKDALE testing completed. Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing ResNet on UKDALE dataset with specific splits...")
    ...
    results = test_resnet_on_all_ukdale_appliances(
        window_size=100, base_width=32, layers=[2, 2, 2],
        epochs=80, lr=0.001, patience=20)

    print(f"\nSummary of ResNet testing on UKDALE dataset:")
