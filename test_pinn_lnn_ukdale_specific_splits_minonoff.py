...
with minimum ON/OFF duration post-processing.

Identical to test_pinn_lnn_ukdale_specific_splits.py with one addition:
after thresholding predictions to a binary ON/OFF mask, a min-ON / min-OFF
filter suppresses physically implausible short events before F1/TP/FP/FN
are computed.  Continuous metrics (MAE, RMSE, SAE) are unaffected.

Min-ON/OFF values (6 s timesteps, derived from NILMTK defaults):
    dish washer  : min_on=300 (30 min)   min_off=300 (30 min)
    fridge       : min_on=10  (1 min)    min_off=2   (12 s)
    microwave    : min_on=2   (12 s)     min_off=5   (30 s)
    washer dryer : min_on=300 (30 min)   min_off=27  (162 s)

Filter logic:
    1. Remove short ON runs  shorter than min_on  → set to OFF
    2. Fill  short OFF gaps  shorter than min_off → set back to ON
"""
...
def apply_min_on_off(binary, min_on, min_off):
    ...
    Args:
        binary:  1-D np.ndarray of int/bool (0=OFF, 1=ON)
        min_on:  minimum consecutive ON steps to keep an activation
        min_off: minimum consecutive OFF steps to keep a deactivation
    Returns:
        Filtered 1-D np.ndarray (same dtype as input)
    """
...
def compute_per_appliance_metrics(y_true, y_pred, y_scalers):
    ...
    for i, app in enumerate(APPLIANCES):
        ...
        # Continuous metrics on unfiltered predictions
        ...
        # Apply min ON/OFF filter to predicted mask only
...
def train_pinn_model(data_dict, save_dir,
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN UKDALE (min ON/OFF) — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
