"""
PINN-LNN UKDALE — v4 for the 'dataset/' splits.

Equivalent to test_pinn_lnn_ukdale_specific_splits_v4.py but reads from
dataset/ (used by the fixed PINN-LNN scripts) instead of new_dataset/.

Changes vs v4 (new_dataset version):
  1. Default --dataset-dir is dataset/ not new_dataset/.
  2. CSV columns match dataset/ naming directly: dishwasher, fridge,
     microwave, washing_machine (no renaming to 'dish washer'/'washer dryer').
  3. Adaptive ON/OFF thresholds replace fixed v4 thresholds — cycling-
     appliance fix prevents fridge threshold from landing above operating power.
  4. TP / TN / FP / FN added to test output alongside F1 / P / R / MAE.

Architecture (unchanged from v4):
  - single-channel input (raw mains, MinMaxScaled)
  - forward-only LiquidCell
  - Seq2Seq output — (batch, WIN, n_apps)
  - physics consistency loss at every timestep
  - no event BCE, no warmup
  - stitched timeline predictions for validation/test metrics
...
# Fridge cycles 0W <-> 100W+ with no standby; p5_nonzero lands inside the
# operating cluster.  Use THRESHOLD_MIN when p5_nonzero exceeds this.
CYCLING_P5_W      = 80.0
...
def create_sequences(df: pd.DataFrame, stride: int):
    ...
    return (np.array(X, np.float32).reshape(-1, WIN, 1),
...
class PhysicsConsistencyLoss(nn.Module):
    ...
    One-sided energy-conservation penalty at every timestep:
        L = mean( ReLU( sum_i p_hat_i_raw[t] - agg_raw[t] - epsilon ) )
    """
...
class PINNLNNSeq2Seq(nn.Module):
    """Forward LiquidCell with per-appliance linear heads; output (B, T, n_apps)."""
...
def train_model(data_dict: dict, save_dir: str,
    ...
    x_sc = MinMaxScaler()
    X_tr = x_sc.fit_transform(X_tr.reshape(-1, 1)).reshape(X_tr.shape)
    X_va = x_sc.transform(    X_va.reshape(-1, 1)).reshape(X_va.shape)
    X_te = x_sc.transform(    X_te.reshape(-1, 1)).reshape(X_te.shape)
    ...
    model     = PINNLNNSeq2Seq(1, hidden_size, len(APPLIANCES), dt).to(device)
    mse_crit  = nn.MSELoss()
...
def _plot_loss(history: dict, save_dir: str) -> None:
    ...
    fig.suptitle('PINN-LNN (old dataset v4) — Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_old_v4_loss.png'),
...
def _plot_metrics(history: dict, test_metrics: dict, save_dir: str) -> None:
    ...
    fig.suptitle('PINN-LNN (old dataset v4) — Stitched Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_old_v4_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_test_trace(test_df: pd.DataFrame, stitched_raw: np.ndarray,
    ...
    fig.suptitle('PINN-LNN (old dataset v4) — Stitched Test Trace', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_old_v4_test_trace.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
def _save_results(save_dir: str, test_metrics: dict,
    ...
    cfg = {
        ...
        'model':       'PINNLNNSeq2Seq (old dataset v4)',
        'description': ('seq2seq PINN-LNN, adaptive thresholds, '
                        'stitched timeline metrics, BCE disabled'),
    ...
    out = os.path.join(save_dir, 'pinn_lnn_old_v4_results.json')
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Train v4 seq2seq PINN-LNN on dataset/ CSV splits.')
...
def main():
    ...
    save_dir  = f'models/pinn_lnn_old_v4_{timestamp}'
