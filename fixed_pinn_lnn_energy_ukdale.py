"""
Fixed PINN-LNN + energy estimation for UKDALE
==============================================================================================
...
Adds final test-set energy estimation by integrating inverse-scaled appliance
power traces and comparing predicted Wh against true Wh.
...
def energy_estimation_metrics(y_true: np.ndarray, y_pred: np.ndarray,
                              y_scalers: list,
                              sample_seconds: float = SAMPLE_PERIOD_SECONDS) -> dict:
    """
    Estimate appliance energy by integrating reconstructed power traces.

    y_true/y_pred are scaled traces with shape (n_samples, n_apps). The values are
    inverse-transformed to watts, clipped at 0 W, then summed as Wh.
    """
    out = {}
    total_true_wh = 0.0
    total_pred_wh = 0.0
    ...
    for i, app in enumerate(APPLIANCES):
        ...
        raw_t = np.clip(raw_t, 0.0, None)
        raw_p = np.clip(raw_p, 0.0, None)

        true_wh = float(raw_t.sum() * sample_seconds / 3600.0)
        pred_wh = float(raw_p.sum() * sample_seconds / 3600.0)
        err_wh = pred_wh - true_wh
        ...
        total_true_wh += true_wh
        total_pred_wh += pred_wh

    total_err_wh = total_pred_wh - total_true_wh
    out['total_labelled'] = {
        'true_wh': total_true_wh,
...
def train(data_dict: dict, save_dir: str,
    ...
    energy_metrics = energy_estimation_metrics(true_trace_te, pred_trace_te, y_scalers)
    ...
    cfg = {
        ...
        'test_energy_metrics': {
            app: {k: float(v) for k, v in m.items()}
            for app, m in energy_metrics.items()
        },
    }
    with open(os.path.join(save_dir, 'fixed_pinn_lnn_energy_results.json'),
...
if __name__ == '__main__':
    ...
    save_dir  = f'models/fixed_pinn_lnn_energy_{timestamp}'
