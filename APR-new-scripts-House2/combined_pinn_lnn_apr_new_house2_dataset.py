"""
combined_pinn_lnn_apr_new_house2_dataset.py
=============================================
Dual-head PINN + Gated-Detection LNN for NILM -- APR-new-House2-dataset/ splits.

Ported from combined_pinn_lnn_ukdale.py (root) with two changes:

1. DATA SOURCE -- default --dataset-dir now points at APR-new-House2-dataset/
...
dataset (see analyze_apr_new_house2_distributions.py) found dishwasher
...
model over test_pinn_lnn_apr_new_house2_dataset.py /
test_pinn_advanced_lnn_apr_new_house2_dataset.py for this dataset.
"""
...
def train(data_dict: dict, save_dir: str,
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house2_dataset_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-new-House2-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house2_dataset_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-new-House2-dataset) -- Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house2_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_apr_new_house2_dataset_{timestamp}'
    )
