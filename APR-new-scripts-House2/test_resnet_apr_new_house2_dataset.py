"""
ResNet (1D) baseline for NILM -- APR-new-House2-dataset/ splits.

Derived from Old-LNN_Algorithms/test_resnet_ukdale_specific_splits.py with the
same three changes described in test_gru_apr_new_house2_dataset.py:
...
the old ResNet script evaluated on whatever state the model was in when the
training loop exited, not necessarily the best epoch.

Architecture/training loop otherwise unchanged: one ResNetModel per appliance
...
def train_resnet_on_appliance(data_dict, appliance_name, window_size=100,
                               ...
                               save_dir='models/resnet_apr_new_house2_dataset'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_apr_new_house2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_apr_new_house2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'resnet_apr_new_house2_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_apr_new_house2_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_apr_new_house2_{appliance_name}_best.pth"),
    ...
    print(f"\nResNet APR-new-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing ResNet on APR-new-House2-dataset...")
    ...
    print(f"\nSummary of ResNet testing on APR-new-House2-dataset:")
