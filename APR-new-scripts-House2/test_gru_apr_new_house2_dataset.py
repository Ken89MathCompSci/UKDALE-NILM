"""
GRU baseline for NILM -- APR-new-House2-dataset/ splits.

Derived from Old-LNN_Algorithms/test_gru_ukdale_specific_splits.py with three changes:

1. DATA SOURCE -- reads directly from APR-new-House2-dataset/ CSV files instead
   ...
   Appliances: dishwasher, fridge, microwave, washing_machine (all House 2 --
   no cross-house domain shift, but House 2 DOES have standby-power artefacts
   on two channels, see point 3).

2. SEQUENCE/TARGET ALIGNMENT FIX -- the old script built X windows at stride 5
   with midpoint targeting inside create_sequences(), then threw that away and
   re-extracted y via `data[appliance].iloc[::5].values[:len(X)]`, which grabs
   the appliance value at the *start* of each window (row i*stride) rather than
   its midpoint (row i*stride + window_size//2). That's a ~50-sample temporal
   misalignment between input window and target. Fixed here by extracting X and
   y together, at the same midpoint-aligned index, in one pass.
...
3. PER-APPLIANCE THRESHOLDS (not the uniform 10 W used for House 1) --
   determined from analyze_apr_new_house2_distributions.py:
       ...
       fridge          : 20 W (idle floor ~9-11 W, compressor ~87-96 W --
                         10 W would read the idle floor as "ON")
       microwave       : 30 W (a ~25-26 W standby/clock-display floor was
                         found held for ~9.7 hours overnight in the training
                         split -- real cooking draws 800-1500 W, so 10 W
                         would misclassify that whole stretch as "ON")
       washing_machine : 10 W (idle standby ~3-4 W, correctly reads OFF)

Also restores the best (lowest val loss) checkpoint before test evaluation --
the old GRU script evaluated on whatever state the model was in when the
training loop exited, not necessarily the best epoch.

Architecture/training loop otherwise unchanged: one GRU model per appliance
...
def train_gru_on_appliance(data_dict, appliance_name, window_size=100,
                            ...
                            save_dir='models/gru_apr_new_house2_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_apr_new_house2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"gru_apr_new_house2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'gru_apr_new_house2_{appliance_name}_history.json'),
...
def test_gru_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"gru_apr_new_house2_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"gru_apr_new_house2_{appliance_name}_best.pth"),
    ...
    print(f"\nGRU APR-new-House2-dataset testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing GRU on APR-new-House2-dataset...")
    ...
    print(f"\nSummary of GRU testing on APR-new-House2-dataset:")
