"""
test_resnet_apr_new_house2_dataset_v2.py
==========================================
v2 of test_resnet_apr_new_house2_dataset.py -- House 2 counterpart to
test_resnet_apr_new_house1_dataset_v2.py, with the same two fixes.
...
per-appliance thresholds instead of House 1's flat 10W):
    ...
    dishwasher       0.7805     0.7266   0.8430     8.77   6.7747
    fridge           0.9113     0.8798   0.9451    20.02  12.7855
    microwave        0.5360     0.4012   0.8072     7.12   6.1731
    washing_machine  0.7448     0.6215   0.9290     9.46   4.6428

Recall is consistently higher than precision across all four appliances,
worst for microwave (P=0.4012 vs R=0.8072) -- the same directional
imbalance-driven pattern already diagnosed for the Transformer/ResNet
House 1 scripts and for House 2's own Transformer v1, just less extreme
here because v1's per-appliance thresholds were already appliance-aware.

FIX 1 -- Gate head (main fix)
    Same as the House 1 v2 script: v1's ResNetModel ends in a single
    unbounded nn.Linear(base_width*4, output_size), trained with MSE only.
    Replace the single fc head with two heads sharing the same conv/
    residual trunk, and predict P(ON) via BCEWithLogits (pos_weight from
    the training ON-fraction, POS_WEIGHT_CLAMP=(1.0, 250.0) -- reused from
...
FIX 2 -- Adaptive per-appliance thresholds (train-locked), replacing the
    ...
    the reported precision/recall/F1.
...
class GatedResNetModel(nn.Module):
    ...
    Identical design to test_resnet_apr_new_house1_dataset_v2.py's
    GatedResNetModel, adapted to House 2's pos_weight clamp.
    """
...
def train_resnet_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                               ...
                               save_dir='models/resnet_apr_new_house2_dataset_v2'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_apr_new_house2_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_apr_new_house2_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'resnet_apr_new_house2_v2_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_apr_new_house2_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_apr_new_house2_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated ResNet v2 APR-new-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated ResNet v2 on APR-new-House2-dataset...")
    ...
    print(f"\nSummary of Gated ResNet v2 testing on APR-new-House2-dataset:")
