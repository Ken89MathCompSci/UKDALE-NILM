"""
Simplified Transformer baseline for NILM -- APR-new-House2-dataset/ splits.

Derived from Old-LNN_Algorithms/test_transformer_ukdale_specific_splits.py with
the same three changes described in test_gru_apr_new_house2_dataset.py:
  1. Reads APR-new-House2-dataset/ CSVs instead of pickled UKDALE splits
     (House 2, train 2013-06-15..07-14 / val 07-15..07-21 / test 07-22..07-28).
  ...
  3. Per-appliance thresholds (not the uniform 10 W used for House 1) --
     determined from analyze_apr_new_house2_distributions.py:
       dishwasher=10 W, fridge=20 W (idle floor ~9-11 W), microwave=30 W
       (a ~25-26 W standby/clock floor was held ~9.7 hours overnight in the
       training split; real cooking draws 800-1500 W), washing_machine=10 W.

Also restores the best (lowest val loss) checkpoint before test evaluation --
the old Transformer script evaluated on whatever state the model was in when
the training loop exited, not necessarily the best epoch.

Architecture/training loop otherwise unchanged: one SimpleTransformerModel
per appliance (sequence-to-point, single output), MSE loss, Adam,
...
def train_transformer_on_appliance(data_dict, appliance_name, window_size=100,
                                    ...
                                    save_dir='models/transformer_apr_new_house2_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"transformer_apr_new_house2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"transformer_apr_new_house2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'transformer_apr_new_house2_{appliance_name}_history.json'),
...
def test_transformer_on_all_appliances(window_size=100, hidden_size=128, num_layers=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"transformer_apr_new_house2_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"transformer_apr_new_house2_{appliance_name}_best.pth"),
    ...
    print(f"\nTransformer APR-new-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Transformer on APR-new-House2-dataset...")
    ...
    print(f"\nSummary of Transformer testing on APR-new-House2-dataset:")
