"""
test_tcn_apr_new_house2_dataset_v2.py
=======================================
v2 of test_tcn_apr_new_house2_dataset.py -- House 2 counterpart to
test_tcn_apr_new_house1_dataset_v2.py, with the same two fixes.

v1 test results:

    Appliance            F1  Precision   Recall      MAE      SAE
    dishwasher       0.6739     0.5817   0.8010    12.26   1.9827
    fridge           0.8953     0.8108   0.9994    20.58   9.6772
    microwave        0.3664     0.2243   1.0000     7.86   2.5218
    washing_machine  0.4580     0.2979   0.9896    12.10   4.9590

Same directional pattern as House 1's TCN and House 2's Transformer/ResNet:
recall is at or near 1.0 for three of the four appliances (fridge=0.9994,
microwave=1.0000, washing_machine=0.9896), while precision lags well
behind, worst for microwave (P=0.2243) and washing_machine (P=0.2979).
Milder than House 1 overall (v1 here already uses per-appliance
thresholds, not House 1's flat 10W), but the same underlying failure mode.

FIX 1 -- Gate head (main fix)
    Same as the House 1 v2 script: v1's TCNModel ends in a single
    unbounded nn.Linear(num_channels[-1], output_size) after global-
    average-pooling the TCN stack, trained with MSE only. Replace the
    single fc head with two heads sharing the same dilated-TCN trunk, and
    predict P(ON) via BCEWithLogits (pos_weight from the training
...
class GatedTCNModel(nn.Module):
    ...
    GatedTCNModel, adapted to House 2's pos_weight clamp.
    """
...
def train_tcn_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                            ...
                            save_dir='models/tcn_apr_new_house2_dataset_v2'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"tcn_apr_new_house2_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_apr_new_house2_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'tcn_apr_new_house2_v2_{appliance_name}_history.json'),
...
def test_tcn_on_all_appliances(window_size=100, num_channels=None, kernel_size=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"tcn_apr_new_house2_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"tcn_apr_new_house2_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated TCN v2 APR-new-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated TCN v2 on APR-new-House2-dataset...")
    ...
    print(f"\nSummary of Gated TCN v2 testing on APR-new-House2-dataset:")
