"""
Random Forest baseline for NILM -- APR-new-House2-dataset/ splits.
...
That transfer-learning machinery does NOT apply here: APR-new-House2-dataset/
...
ResNet/Transformer family in this folder. Concretely:
  ...
  - One RandomForestRegressor per appliance predicting continuous power (W)
    ...
    test_gru_apr_new_house2_dataset.py etc.
...
def train_rf_on_appliance(data_dict, appliance_name, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'rf_apr_new_house2_{appliance_name}_feature_importance.png'),
    ...
    with open(os.path.join(save_dir, f'rf_apr_new_house2_{appliance_name}_results.json'),
...
def main():
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"rf_apr_new_house2_dataset_{timestamp}")
    ...
    print(f"\nRandom Forest APR-new-House2-dataset testing completed. Results saved to {base_save_dir}\n")
