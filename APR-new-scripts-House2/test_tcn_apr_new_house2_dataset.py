"""
TCN (Temporal Convolutional Network) baseline for NILM -- APR-new-House2-dataset/ splits.

Derived from Old-LNN_Algorithms/test_tcn_ukdale_specific_splits.py with the
...
the old TCN script evaluated on whatever state the model was in when the
training loop exited, not necessarily the best epoch.

Architecture/training loop otherwise unchanged: one TCNModel per appliance
...
def train_tcn_on_appliance(data_dict, appliance_name, window_size=100,
                            ...
                            save_dir='models/tcn_apr_new_house2_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"tcn_apr_new_house2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_apr_new_house2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'tcn_apr_new_house2_{appliance_name}_history.json'),
...
def test_tcn_on_all_appliances(window_size=100, num_channels=None, kernel_size=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"tcn_apr_new_house2_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"tcn_apr_new_house2_{appliance_name}_best.pth"),
    ...
    print(f"\nTCN APR-new-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing TCN on APR-new-House2-dataset...")
    ...
    print(f"\nSummary of TCN testing on APR-new-House2-dataset:")
