"""
combined_pinn_lnn_apr_new_house2_dataset_v2.py
===============================================
v2 of combined_pinn_lnn_apr_new_house2_dataset.py -- two targeted fixes.

Context: Combined-PINN-LNN on House 2 already leads on F1/Precision/MAE/SAE
vs TCN/LNN/RESNET baselines, but its recall (0.85) trails them (0.88-0.94).
That gap is structural -- the gate mechanism trades some recall for large
precision gains.  These fixes recover recall where training is provably
sub-optimal without touching the gate architecture itself.

FIX 2 -- pos_weight clamp (raise upper bound for severely imbalanced appliances)
    House 2 microwave is ON ~0.4% of training samples, so the true
    n_off/n_on ratio is ~249.  The original POS_WEIGHT_CLAMP=(1.0, 50.0)
    truncates this to 50, making the gate ~5x under-weighted for microwave
    (which has the worst recall of any House 2 appliance, R=0.768).
    Fix: raise the upper clamp to 250 so the true imbalance ratio is used.
    Each epoch's diagnostic now prints both the raw ratio and the clamped
    value, flagging appliances where truncation was occurring.
    Expected effect: microwave recall increases; some precision trades back.

    Note: Fix 1 (threshold consistency) does NOT apply to House 2 -- the
    cycling exception in compute_adaptive_thresholds never fires for any H2
    appliance (all p5 values are below CYCLING_P5_W=80W), so the three
    threshold functions already agree across splits here.

FIX 3 -- Lambda ramp (smooth loss transition after warmup)
    The original code switches from MSE-only to MSE+physics+event abruptly at
    epoch WARMUP_EPOCHS.  House 2 logs show the same whiplash as House 1:
    avgF1 drops from 0.910 (epoch 20, end of warmup) to 0.753 at epoch 21,
    with dishwasher R falling 0.99->0.769 and microwave R falling 0.976->0.768
    -- neither recovering to their warmup ceiling by the end of training.
    Fix: linearly ramp lambda from 0 -> 1 over RAMP_EPOCHS epochs after warmup.
        lam = min((epoch - WARMUP_EPOCHS + 1) / RAMP_EPOCHS, 1.0)
        loss = l_mse + lam * (lambda_phys * l_phys + lambda_event * l_ev)
    Phase label in epoch log: WARMUP -> RAMP(0.xx) -> FULL.
    Expected effect: dishwasher and microwave recall recover closer to their
    warmup peaks; fridge and washing_machine mostly unchanged (already near
    warmup ceiling in v1).

Everything else -- architecture, losses, thresholds, scalers, Seq2Seq
construction -- is unchanged from v1.
"""
...
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 80.0
# FIX 2: upper clamp raised from 50 -> 250 to cover microwave's ~249:1 imbalance
POS_WEIGHT_CLAMP  = (1.0, 250.0)
...
def train(data_dict: dict, save_dir: str,
    ...
    print(f"  FIX 1 -- N/A for House 2 (cycling threshold exception never fires)")
    print(f"  FIX 2 -- pos_weight upper clamp raised 50 -> 250 (microwave ~249:1 imbalance)")
    ...
    # FIX 2: pos_weight -- print raw ratio alongside clamped value so truncation is visible
    pos_weights = []
    print("\n  Gate pos_weight per appliance  [clamp upper=250, was 50]:")
    print(f"  {'Appliance':<16} {'on%':>6}  {'raw ratio':>10}  {'clamped':>8}  note")
    for i, app in enumerate(APPLIANCES):
        flat    = Y_tr[:, :, i].flatten()
        ...
        was_clipped = " <-- was clamped to 50 in v1" if raw_pw > 50.0 else ""
        print(f"  {app:<16} {100*n_on/(n_on+n_off):>6.2f}%  {raw_pw:>10.1f}  {pw:>8.1f}{was_clipped}")
    ...
    cfg = {
        'version': 'v2',
        'fixes': {
            'fix1_threshold_consistency': 'N/A for House 2 -- cycling exception never fires',
            'fix2_pos_weight_clamp':      'upper clamp raised 50->250 (microwave ~249:1 raw ratio)',
        ...
        'train_params': {'lr': LR, 'epochs': EPOCHS, 'patience': PATIENCE,
                         ...
                         'pos_weight_clamp': list(POS_WEIGHT_CLAMP)},
        'test_metrics': {app: {k: float(v) for k, v in m.items()}
                         for app, m in test_metrics.items()},
    }
    with open(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house2_dataset_v2_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN v2 (APR-new-House2-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house2_dataset_v2_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN v2 (APR-new-House2-dataset) -- Per-Appliance Val Metrics')
    for row, app in enumerate(APPLIANCES):
        ...
        for ax, vals, label, color in [
            ...
            (axes[row][1], maes, 'MAE (W)', 'tomato'),
            (axes[row][2], saes, 'SAE',     'purple'),
        ]:
            ax.plot(ep, vals, color=color)
            ax.axvspan(1, WARMUP_EPOCHS, alpha=0.05, color='gray')
            ax.axvspan(WARMUP_EPOCHS + 1, WARMUP_EPOCHS + RAMP_EPOCHS,
                       alpha=0.05, color='orange')
            ax.axhline(test_metrics[app][label.split()[0].lower()],
                       color='green', linestyle='--', label='Test')
            ax.set_title(f'{app} -- {label}'); ax.legend(fontsize=7); ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_apr_new_house2_dataset_v2_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_apr_new_house2_dataset_v2_{timestamp}'
    )
