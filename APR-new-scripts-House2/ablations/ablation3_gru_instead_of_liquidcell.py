"""
ablation3_gru_instead_of_liquidcell.py  (House 2)
===================================================
Tier 1 ablation #3 of combined_pinn_lnn_apr_new_house2_dataset_v2.py: swap the
custom LiquidCell (variable time-constant "liquid" dynamics) for a
standard nn.GRUCell of the same hidden size.

WHAT CHANGED vs the v2 baseline
---------------------------------
Baseline:  self.fwd_cell = LiquidCell(hidden, hidden, dt)
           self.bwd_cell = LiquidCell(hidden, hidden, dt)
           -- ODE-inspired update: dh = ((-h/tau) + g*tanh(ip+rp)) * dt,
              where tau is a per-unit, input-modulated time constant.
Ablation:  self.fwd_cell = nn.GRUCell(hidden, hidden)
           self.bwd_cell = nn.GRUCell(hidden, hidden)
           -- standard gated recurrent update, no notion of a continuous
              time constant.

nn.GRUCell has the same call signature as LiquidCell.forward(x_t, h) --
h_new = cell(x_t, h) -- so this is close to a drop-in swap. `dt` is no
longer used by the recurrent cell (kept as an unused constructor arg only
so the rest of the script's signatures don't have to change).

Everything else -- CNN front-end, bidirectional structure, gate mechanism,
GatedEventLoss, physics loss, 8-channel features, thresholds,
checkpoint-on-val-MSE, WIN=299, POS_WEIGHT_CLAMP=(1.0, 250.0),
RAMP_EPOCHS=10 -- is UNCHANGED from the v2 baseline, so this isolates
exactly one variable: whether the "liquid" (ODE-style, input-modulated tau)
recurrence is actually earning its complexity over a standard, cheaper GRU.

WHY THIS ABLATION
------------------
"LNN" (Liquid Neural Network) is in this project's name and this cell is
its most novel/distinctive component vs. a conventional RNN-based NILM
model. This is the ablation most worth running to check whether that
architectural choice is doing real work, or whether a plain GRU -- fewer
parameters, no dt/tau machinery, well-understood training dynamics --
gets equivalent results.
"""
...
# ---------------------------------------------------------------------------
# Constants  (identical to v2 baseline)
# ---------------------------------------------------------------------------
...
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 80.0
POS_WEIGHT_CLAMP  = (1.0, 250.0)   # v2: raised from 50 -> 250 for microwave imbalance
...
# LiquidCell is intentionally NOT defined in this file -- this ablation
# replaces it with nn.GRUCell below, which has the same forward(x_t, h)
# call signature.


# ---------------------------------------------------------------------------
# Combined model  -- GRUCell instead of LiquidCell
# ---------------------------------------------------------------------------

class CombinedPINNGRU(nn.Module):
    """
    Ablation of CombinedPINNAdvancedLNN with LiquidCell replaced by
    nn.GRUCell (same hidden size). Everything else -- CNN front-end,
    bidirectional structure, gate mechanism -- is unchanged from v2 baseline.
    """
    ...
    def __init__(self, in_ch: int, hidden: int, n_apps: int, dt: float = 0.1):
        ...
        self.hidden = hidden
        self.n_apps = n_apps
        # dt is accepted for call-signature parity with the baseline but is
        # unused here -- GRUCell has no continuous time-constant notion.
        self.dt = dt

        self.cnn      = LengthPreservingCNN(in_ch, hidden)
        self.fwd_cell = nn.GRUCell(hidden, hidden)
        self.bwd_cell = nn.GRUCell(hidden, hidden)
...
# ---------------------------------------------------------------------------
# Losses  (identical to v2 baseline)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Training  (identical loss schedule to v2 baseline)
# ---------------------------------------------------------------------------
...
def train(data_dict: dict, save_dir: str,
    ...
    print(f"Model: CombinedPINNGRU  [ABLATION 3: LiquidCell -> nn.GRUCell]\n")
    ...
    # pos_weight from training ON fraction (clamp upper=250 from v2)
    pos_weights = []
    print("\n  Gate pos_weight per appliance  [clamp upper=250]:")
    for i, app in enumerate(APPLIANCES):
        flat  = Y_tr[:, :, i].flatten()
    ...
    model       = CombinedPINNGRU(n_feat, hidden, len(APPLIANCES), dt).to(device)
    mse_crit    = nn.MSELoss()
    ...
    cfg = {
        'ablation': 'ablation3_gru_instead_of_liquidcell',
        'ablation_description': 'Replaced LiquidCell (ODE-inspired, input-modulated tau) with '
                                'nn.GRUCell of the same hidden size, for both the forward and '
                                'backward recurrent passes. Gate and loss schedule identical to v2 baseline.',
        'dataset': 'APR-new-House2-dataset (H2 train 2013-06-15..07-14 / val 07-15..07-21 / test 07-22..07-28)',
        'model':   'CombinedPINNGRU',
        ...
        'model_params': {'in_ch': n_feat, 'hidden': hidden, 'n_apps': len(APPLIANCES)},
    ...
    with open(os.path.join(save_dir, 'ablation3_gru_instead_of_liquidcell_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('Ablation 3: GRUCell instead of LiquidCell (APR-new-House2-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'ablation3_gru_instead_of_liquidcell_loss.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('Ablation 3: GRUCell instead of LiquidCell (APR-new-House2-dataset) -- Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'ablation3_gru_instead_of_liquidcell_per_appliance.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'ablation3_gru_instead_of_liquidcell_house2_{timestamp}'
    )
