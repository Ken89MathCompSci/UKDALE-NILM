"""
ablation4_raw_feature_only.py  (House 2)
==========================================
Tier 1 ablation #4 of combined_pinn_lnn_apr_new_house2_dataset_v2.py: feed the
model only the raw aggregate power channel, dropping the other 7 engineered
channels.

WHAT CHANGED vs the v2 baseline
---------------------------------
Baseline:  compute_features() returns an 8-channel stack:
           [raw, median-filtered, EMA-smoothed, residual, diff(raw),
            diff(smooth), rolling mean, rolling std]
Ablation:  compute_features() returns a 1-channel stack: [raw] only.

No other code changes are needed for this ablation -- n_feat is derived
dynamically from X_tr.shape[2] at training time (see train()), so the CNN's
in_ch automatically becomes 1 instead of 8. Architecture, losses,
thresholds, checkpoint-on-val-MSE, WIN=299, POS_WEIGHT_CLAMP=(1.0, 250.0),
RAMP_EPOCHS=10 are all otherwise UNCHANGED from the v2 baseline, so this
isolates exactly one variable: whether the hand-engineered channels
(median/EMA smoothing, residual, deltas, rolling stats) are earning their
keep over letting the CNN learn those transformations itself from raw power.

WHY THIS ABLATION
------------------
7 of the baseline's 8 input channels are deterministic functions of the
8th (raw power) -- median filter, EMA, their difference, two first-order
diffs, and two rolling statistics. A 4-layer dilated CNN (receptive field
183 steps, see LengthPreservingCNN) is easily capable of learning
smoothing- and difference-like operations itself. This ablation checks
whether the explicit feature engineering measurably helps generalization
(e.g. by giving the model a more direct signal for rare events) or is
redundant compute that the network would learn anyway.
"""
...
# ---------------------------------------------------------------------------
# 1-channel feature extraction  -- ABLATED (v2 baseline had 8 channels)
# ---------------------------------------------------------------------------

def compute_features(mains: np.ndarray) -> np.ndarray:
    """
    ABLATION 4: raw aggregate power only. Baseline's compute_features()
    also returned median-filtered, EMA-smoothed, residual, two diffs, and
    two rolling-window statistics (7 additional channels) -- all dropped
    here. Shape (N, 1) instead of (N, 8); n_feat is derived dynamically
...
# ---------------------------------------------------------------------------
# Model components  (identical to v2 baseline)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Combined model  (identical to v2 baseline -- only its input channel count changes)
# ---------------------------------------------------------------------------

class CombinedPINNAdvancedLNN(nn.Module):
    """
    Same architecture as the v2 baseline's CombinedPINNAdvancedLNN. Only
    compute_features() above changed (8 channels -> 1); in_ch is derived
    dynamically from the data so this class itself is unmodified.
    """
...
def train(data_dict: dict, save_dir: str,
    ...
    print(f"Model: CombinedPINNAdvancedLNN  [ABLATION 4: raw feature only, 1 input channel]\n")
    ...
    print(f"  n_feat = {n_feat}  (v2 baseline uses 8; this ablation uses 1)")
    ...
    cfg = {
        'ablation': 'ablation4_raw_feature_only',
        'ablation_description': 'compute_features() returns raw aggregate power only '
                                '(1 channel) instead of the v2 baseline 8-channel stack '
                                '(raw, median, EMA-smooth, residual, 2 diffs, rolling mean/std). '
                                'Gate and loss schedule identical to v2 baseline.',
    ...
    with open(os.path.join(save_dir, 'ablation4_raw_feature_only_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('Ablation 4: Raw Feature Only (APR-new-House2-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'ablation4_raw_feature_only_loss.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('Ablation 4: Raw Feature Only (APR-new-House2-dataset) -- Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'ablation4_raw_feature_only_per_appliance.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'ablation4_raw_feature_only_house2_{timestamp}'
    )
