"""
ablation2_unidirectional_lnn.py  (House 2)
===========================================
Tier 1 ablation #2 of combined_pinn_lnn_apr_new_house2_dataset_v2.py: drop the
backward LiquidCell pass, keeping only the forward (causal) direction.

WHAT CHANGED vs the v2 baseline
---------------------------------
Baseline:  h_t = LayerNorm(hidden*2)(concat(fwd[t], bwd[t]))
           gate_heads / power_heads: Linear(hidden*2, 1)
Ablation:  h_t = LayerNorm(hidden)(fwd[t])                    -- no bwd_cell
           gate_heads / power_heads: Linear(hidden, 1)

Everything else -- CNN front-end, LiquidCell itself, gate mechanism,
...
exactly one variable: whether future context (the backward pass) matters.

WHY THIS ABLATION
------------------
The baseline reads the whole WIN=299-step window with both forward and
backward LiquidCells before making a prediction at any timestep t -- i.e.
every prediction can see up to ~30 minutes into the future within the
window. That's fine for offline batch evaluation but would not be usable
in a real-time/streaming NILM system, which can only be causal. This
ablation tests how much accuracy the non-causal (future-seeing) half of
the model is actually buying, and by extension how much accuracy a
real-time-capable version of this model would have to give up.
"""
...
# ---------------------------------------------------------------------------
# Ablated model  -- UNIDIRECTIONAL: forward LiquidCell pass only
# ---------------------------------------------------------------------------

class UnidirectionalPINNLNN(nn.Module):
    """
    Ablation of CombinedPINNAdvancedLNN with the backward LiquidCell pass
    removed. Prediction at timestep t can only see feat[0..t] (causal),
    not the whole window. Gate mechanism and dual heads are unchanged from
    the v2 baseline -- only the recurrent context is halved.
    """
    ...
    def __init__(self, in_ch: int, hidden: int, n_apps: int, dt: float = 0.1):
        ...
        # No bwd_cell in this ablation.
        self.norm     = nn.LayerNorm(hidden)

        self.gate_heads  = nn.ModuleList([nn.Linear(hidden, 1) for _ in range(n_apps)])
        self.power_heads = nn.ModuleList([nn.Linear(hidden, 1) for _ in range(n_apps)])
...
def train(data_dict: dict, save_dir: str,
    ...
    print(f"Model: UnidirectionalPINNLNN  [ABLATION 2: no backward LiquidCell pass]\n")
    ...
    model       = UnidirectionalPINNLNN(n_feat, hidden, len(APPLIANCES), dt).to(device)
    mse_crit    = nn.MSELoss()
    ...
    cfg = {
        'ablation': 'ablation2_unidirectional_lnn',
        'ablation_description': 'Removed the backward LiquidCell pass -- forward-only (causal) '
                                'recurrence. h_t = LayerNorm(hidden)(fwd[t]) instead of '
                                'LayerNorm(hidden*2)(concat(fwd[t], bwd[t])). '
    ...
    with open(os.path.join(save_dir, 'ablation2_unidirectional_lnn_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('Ablation 2: Unidirectional LNN (APR-new-House2-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'ablation2_unidirectional_lnn_loss.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('Ablation 2: Unidirectional LNN (APR-new-House2-dataset) -- Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'ablation2_unidirectional_lnn_per_appliance.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'ablation2_unidirectional_lnn_house2_{timestamp}'
    )
