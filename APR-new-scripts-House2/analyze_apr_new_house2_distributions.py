...
Splits (all House 2, chronologically contiguous, no gap/overlap):
    train      2013-06-15 to 2013-07-14 (30 days)
    validation 2013-07-15 to 2013-07-21 (7 days)
    test       2013-07-22 to 2013-07-28 (7 days)

House 2 sensor artefacts that affect thresholds:
    fridge           -- never reads 0 W; idle floor ~9-11 W, compressor ~87-96 W.
                        A 20 W threshold is used here to detect compressor cycles
                        rather than the standard 10 W (which would give 100% ON).
    washing_machine  -- idle standby ~3-4 W; a 10 W threshold correctly reads OFF.
    microwave        -- true 0 W when off; cleanest channel across all houses.
    dishwasher       -- off = 0-1 W; same behaviour as House 1.
...
THRESHOLDS   = {
    'dishwasher':      10.0,
    'fridge':          20.0,   # idle floor ~9-11 W; 20 W detects compressor cycling
    'microwave':       10.0,
...
def plot_comparison(all_results, out_dir=DATASET_DIR):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2, figsize=(14, 4 * len(APPLIANCES)))
    fig.suptitle('APR-new-House2-dataset -- Train vs Validation vs Test comparison',
                 fontsize=13)
