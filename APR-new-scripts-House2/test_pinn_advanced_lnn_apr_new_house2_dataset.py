"""
Physics-Informed Advanced LNN (PINN-AdvancedLNN) for NILM — APR-new-House2-dataset splits.
...
1. DATA SOURCE — reads directly from APR-new-House2-dataset/ CSV files instead
   ...
   All three splits are House 2, chronologically contiguous — no cross-house
   domain shift, but House 2 DOES have standby-power artefacts on two
   channels (see point 2).
   Columns: timestamp, aggregate, dishwasher, fridge, microwave, washing_machine

2. PER-APPLIANCE THRESHOLDS (not the uniform 10 W used for House 1) --
...
vs. the single AdvancedLiquidTimeLayer used in test_pinn_lnn_apr_new_house2_dataset.py):
...
def train_pinn_model(data_dict, save_dir,
    ...
    with open(os.path.join(save_dir, 'pinn_advanced_lnn_apr_new_house2_dataset_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_advanced_lnn_apr_new_house2_dataset_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-AdvancedLNN APR-new-House2-dataset -- Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_advanced_lnn_apr_new_house2_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_advanced_lnn_apr_new_house2_dataset_{timestamp}"
    )
