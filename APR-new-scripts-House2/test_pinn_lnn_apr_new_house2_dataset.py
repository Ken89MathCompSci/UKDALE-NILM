"""
Physics-Informed LNN (PINN-LNN) for NILM — APR-new-House2-dataset splits.

Derived from test_pinn_lnn_apr_dataset.py with one change:

1. DATA SOURCE — reads directly from APR-new-House2-dataset/ CSV files:
       APR-new-House2-dataset/UKDALE_HF_train.csv      (House 2, 2013-06-15 to 2013-07-14, 30 days)
       APR-new-House2-dataset/UKDALE_HF_validation.csv (House 2, 2013-07-15 to 2013-07-21, 7 days)
       APR-new-House2-dataset/UKDALE_HF_test.csv       (House 2, 2013-07-22 to 2013-07-28, 7 days)
   All three splits are House 2, chronologically contiguous (no gap/overlap
   between them) — no cross-house domain shift, but House 2 DOES have
   standby-power artefacts on two channels (see point 3).
   Columns: timestamp, aggregate, dishwasher, fridge, microwave, washing_machine

2. LOSS REWEIGHTING MACHINERY (ported as-is from
   test_pinn_lnn_apr_new_house1_dataset.py) — the shared-encoder joint MSE
   can let an ever-present appliance (e.g. a fridge that's ON ~50% of the
   time) drown out a rare one's gradient (e.g. a dishwasher with several
   zero-activity training days). Two mitigations, both inherited unchanged:
     a) Per-appliance MSE loss weighting -- weights start as
        sqrt(1 / train_on_rate), normalized to mean 1.0, computed at runtime
        from whichever training split is actually loaded (see MSE_WEIGHTS in
        train_pinn_model), so this recomputes correctly for House 2's own
        on-rates rather than reusing House 1's numbers. Every appliance
        except fridge is floored at 1.0 so amplifying the rare appliances
        can't drag a moderately-rare one (e.g. washing_machine) below its
        original contribution -- see that script's docstring for the H1 run
        where the un-floored version regressed washing_machine's F1.
        Validation/early-stopping still uses the plain (unweighted) MSE so
        model selection isn't skewed by the same reweighting.
     b) Dishwasher's BCE_LAMBDA/BCE_ALPHA were raised (0.5->1.0 / 2.0->4.0)
        based on House 1's dishwasher recall problem. Kept as the starting
        point here too, but this was tuned on House 1 data and not
        re-verified against House 2 -- may need its own tuning pass.

3. PER-APPLIANCE THRESHOLDS (not the uniform 10 W used for House 1) --
   determined from analyze_apr_new_house2_distributions.py:
       dishwasher      : 10 W (clean channel, off = 0-1 W)
       fridge          : 20 W (idle floor ~9-11 W, compressor ~87-96 W)
       microwave       : 30 W (a ~25-26 W standby/clock-display floor was
                         found held for ~9.7 hours overnight in the training
                         split; real cooking draws 800-1500 W)
       washing_machine : 10 W (idle standby ~3-4 W, correctly reads OFF)
...
# Per-appliance thresholds -- House 2 has standby-power artefacts on fridge
# and microwave that a uniform 10 W would misclassify (see docstring).
THRESHOLDS = {
    'dishwasher':      10.0,
    'fridge':          20.0,   # idle floor ~9-11 W
    'microwave':       30.0,   # ~25-26 W overnight standby/clock floor
    'washing_machine': 10.0,
}

# Dishwasher raised (0.5->1.0 / 2.0->4.0): carried over from House 1, where
# 40% of training days had zero dishwasher activity and needed stronger BCE
# pressure to recover recall. Not re-verified against House 2's own
# dishwasher activity pattern -- may need its own tuning pass.
...
def load_data(dataset_dir=DATASET_DIR):
    """Load train / validation / test from APR-new-House2-dataset/ CSV files."""
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        ...
        'model': 'PhysicsInformedLiquidNetworkModel',
        'description': 'adaptive LNN (adaptive tau + input gate) + L_phys; all House 2, per-appliance thresholds (see thresholds field)',
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_apr_new_house2_dataset_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_apr_new_house2_dataset_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN APR-new-House2-dataset -- Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_apr_new_house2_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_lnn_apr_new_house2_dataset_{timestamp}"
    )
